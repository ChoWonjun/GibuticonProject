	...
	$(document).ready(function(){
		...
		$("#coneValSel").change(function(){
			...
			if($("#coneValSel").val()==""){
				...
				chargeCone.coneVal.value=$("#coneValSel").val();
				chargeCone.payVal.value=chargeCone.coneVal.value*100;
			}
		});
		$("#selfConeVal").keyup(function(){
			if(isNaN($("#selfConeVal").val())){
				$("#selfConeVal").val("");
				alert("숫자로만 입력하세요!");
			}
			chargeCone.coneVal.value=$("#selfConeVal").val();
			chargeCone.payVal.value=chargeCone.coneVal.value*100;
		});
...
<span id="coneInput">직접입력 <input type=text id=selfConeVal>cone</span>
<br>
<hr>
결제금액 <input type="text" disabled="disabled" name="coneVal" id="coneVal">
X100원=<input type="text" disabled="disabled" name="payVal">원<hr>
결제수단
<input type="radio" name="payment" value="mobile"> 휴대폰
<input type="radio" name="payment" value="transfer"> 계좌이체
<input type="radio" name="payment" value="credit"> 신용카드<hr>
<input type="button" value="충전" id="chargeButton"> <input type="button" value="취소" onclick="javascript:window.close()">
</form>
</body>
</html>