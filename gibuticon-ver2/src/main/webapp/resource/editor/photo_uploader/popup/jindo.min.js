...
/**
 * 
 * @function $ 함수는 다음의 두 가지 역할을 한다.
 *           <ul>
 *           <li/>ID를 사용하여 HTML 엘리먼트를 가져온다. 매개변수를 두 개 이상 지정하면 HTML 엘리먼트를 원소로하는
 *           배열을 리턴한다.
 *           <li>또한 "<tagName>" 과 같은 형식의 문자열을 입력하면 tagName을 가지는 객체를 생성한다.</li>
 *           </ul>
 * @param {String...}
 *            sID HTML 엘리먼트의 ID. ID는 하나 이상 지정할 수 있다. (1.4.6부터는 마지막 매개변수에
 *            document을 지정할수 있다.)
 * @return {Element|Array} HTML 엘리먼트 혹은 HTML 엘리먼트를 원소로 가지는 배열을 리턴한다. 만약 ID에 해당하는
 *         HTML 엘리먼트가 없으면 null을 리턴한다.
 * @description [Lite]
 * @example // ID를 이용하여 객체를 리턴한다. <div id="div1"></div>
 * 
 * var el = $("div1");
 *  // ID를 이용하여 여러개의 객체를 리턴한다. <div id="div1"></div> <div id="div2"></div>
 * 
 * var els = $("div1","div2"); // [$("div1"),$("div2")]와 같은 결과를 리턴한다.
 *  // tagName과 같은 형식의 문자열을 이용하여 객체를 생성한다. var el = $("<DIV>"); var els = $("<DIV
 * id='div1'><SPAN>hello</SPAN></DIV>");
 * 
 * //IE는 iframe에 추가할 엘리먼트를 생성하려고 할 때는 document를 반드시 지정해야 한다.(1.4.6 부터 지원) var
 * els = $("<div>" , iframe.contentWindow.document); //위와 같을 경우 div태그가
 * iframe.contentWindow.document기준으로 생김.
 * 
 */
 ...
 * 
 * @extends core
 * @class $Class는 Jindo에서 객체 지향 프로그래밍(OOP)를 구현하는 객체이다. $Class.$init 메소드는 클래스를
 *        생성할 때 클래스 인스턴스에 대한 생성자 함수를 정의한다.
 * @param {Object}
 *            oDef 클래스를 정의하는 객체. 메서드, 프로퍼티와 생성자를 정의한다. $staic 키워드는 인스턴스를 생성하지
 *            않아도 사용할 수 있는 메서드의 집합이다.
 * @return {$Class} 클래스 객체
 * @description [Lite]
 * @example var CClass = $Class({ prop : null, $init : function() { this.prop =
 *          $Ajax(); ... }, $static : { static_method : function(){ return 1;} }
 *          });
 * 
 * var c1 = new CClass(); var c2 = new CClass(); // c1과 c2는 서로 다른 $Ajax 객체를 각각
 * 가진다.
 * 
...
jindo.$Class = function(oDef) {
	function typeClass() {
		...
		var superFunc = function(m, superClass, func) {
			...
			if (m != 'constructor' && func.toString().indexOf("$super") > -1) {
				...
				// var funcStr =
				// func.toString().replace(/function\s*\(.*\)\s*\{/,"").replace(/this\.\$super/g,"this.$super.$super");
	...
	// } else {
	// oDef.$destroy =
	// eval("false||"+oDef.$destroy.toString().replace(/\}$/,"console.log(this.$super);console.log(arguments.callee!=this.$super.$destroy);if(this.$super&&(arguments.callee==this.$destroy)){this.$super.$destroy();}}"));
	// }
	// 
...
/**
 * 
 * 클래스를 상속한다. 하위 클래스는 this.$super.method 로 상위 클래스의 메서드에 접근할 수 있으나,
 * this.$super.$super.method 와 같이 한 단계 이상의 상위 클래스는 접근할 수 없다.
 * 
 * @name $Class#extend
 * @type $Class
 * @function
 * @param {$Class}
 *            superClass 수퍼 클래스 객체
 * @return {$Class} 상속된 클래스
 * @description [Lite]
 * @example var ClassExt = $Class(classDefinition); ClassExt.extend(superClass); //
...
/**
 * 
 * 부모 클래스의 메서드에 접근할 때 사용한다. 부모 클래스와 자식 클래스가 같은 이름의 메서드를 가지고 있고 $super로 그 메서드를
 * 호출하면, 자식 클래스의 메서드를 사용한다.
 * 
 * @name $Class#$super
 * @type $Class
 * @example var Parent = $Class ({ a: 100, b: 200, c: 300, sum2: function () {
 *          var init = this.sum(); return init; }, sum: function () { return
 *          this.a + this.b } });
 * 
 * var Child = $Class ({ a: 10, b: 20, sum2 : function () { var init =
 * this.sum(); return init; }, sum: function () { return this.b; } }).extend
 * (Parent);
 * 
 * var oChild = new Child(); var oParent = new Parent();
 * 
 * oChild.sum(); // 20 oChild.sum2(); // 20 oChild.$super.sum(); // 30 -> 부모
 * 클래스의 100(a)과 200(b)대신 자식 클래스의 10(a)과 20(b)을 더한다. oChild.$super.sum2(); // 20 ->
 * 부모 클래스의 sum2 메서드에서 부모 클래스의 sum()이 아닌 자식 클래스의 sum()을 호출한다.
 * 
 */
/**
 * 
 ...
 *            CSS셀렉터
 * @param {Element}
 *            탐색 대상이 되는 요소, 요소의 하위 노드에서만 탐색한다.
 * @return {Array} 조건에 해당하는 요소의 배열을 반환한다.
 * @description [Lite]
 * @example // 문서에서 IMG 태그를 찾는다. var imgs = $$('IMG');
 *  // div 요소 하위에서 IMG 태그를 찾는다. var imgsInDiv = $$('IMG', $('div'));
 *  // 문서에서 IMG 태그 중 가장 첫 요소를 찾는다. var firstImg = $$.getSingle('IMG');
 * 
 */
jindo.$$ = jindo.cssquery = (function() {
			...
			aRet = (function(sQuery, oParent, oOptions) {
				...
				for (var i = 0; i < nLen; i++) {
					...
					if (aCache) {
						...
						/*
						 * 
						 * 캐싱되어 있는게 있으면 parent 가 같은건지 검사한후 aSingleQueryResult 에
						 * 대입
						 * 
						 */
	 ...
	 * @param {Element}
	 *            element 검사하고자 하는 엘리먼트
	 * @param {String}
	 *            selector CSS 셀렉터
	 * @return {Boolean} 셀렉터 조건에 부합하면 true, 부합하지 않으면 false
	 * @example
	 *  // oEl 이 div 태그 또는 p 태그, 또는 align=center 인 엘리먼트인지 if
	 * (cssquery.test(oEl, 'div, p, [align=center]')) alert('해당 조건 만족');// oEl 이
	 * div 태그 또는 p 태그, 또는 align=center 인 엘리먼트인지 if (cssquery.test(oEl, 'div, p,
	 * [align=center]')) alert('해당 조건 만족');
	 * 
	 */
	 ...
	 * 
	 * @remark 결과 캐쉬를 사용하면 동일한 셀렉터를 사용했을 경우 새로 탐색을 하지 않고 기존 탐색 결과를 그대로 반환하기 때문에
	 *         사용자가 변수 캐쉬에 신경쓰지 않고 편하고 빠르게 쓸 수 있는 장점이 있지만 결과의 신뢰성을 위해 DOM 에 변화가
	 *         없다는 것이 확실할때만 사용해야 한다.
	 * @param {Boolean}
	 *            flag 사용할 것 인지 여부 (생략시 사용 여부만 반환)
	 ...
	 * 
	 * @remark 결과를 하나만 얻어내면 이후의 모든 탐색 작업을 중단하기 때문에 결과가 하나라는 보장이 있을때 빠른 속도로 결과를
	 *         얻어올 수 있다.
	 * @param {String}
	 *            selector CSS 셀렉터
	 * @param {Document |
	 *            Element} el 탐색을 진행하는 기준이 되는 엘리먼트 또는 문서 (생략시 현재 문서의 document
	 *            객체)
	 * @param {Object}
	 *            오브젝트에 onTimeOffCache를 true로 하면 해당 쿼리는 cache를 사용하지 않는다.
	 ...
	 * 
	 * @param {Function}
	 *            callback 셀렉터 실행에 소요된 비용과 시간을 받아들이는 함수 (false 인 경우 debug 옵션을 끔)
	 * @param {Number}
	 *            repeat 하나의 셀렉터를 반복하여 수행하도록 해서 인위적으로 실행 속도를 늦춤
	 * @remark callback 함수의 형태는 아래와 같습니다. callback : function({String}query,
	 *         {Number}cost, {Number}executeTime)
	 *         <dl>
	 *         <dt>query</dt>
	 *         <dd>실행에 사용된 셀렉터</dd>
	 *         <dt>cost</dt>
	 ...
	 * @example
	 * 
	 * cssquery.debug(function(sQuery, nCost, nExecuteTime) { if (nCost > 5000)
	 * console.warn('5000 이 넘는 비용이?! 체크해보자 -> ' + sQuery + '/' + nCost); else if
	 * (nExecuteTime > 200) console.warn('0.2초가 넘게 실행을?! 체크해보자 -> ' + sQuery +
	 * '/' + nExecuteTime); }, 20);
	 * 
	 * ....
	 * 
	...
	/**
	 * 
	 * IE 에서 innerHTML 을 쓸때 _cssquery_UID 나오지 않도록 하는 함수이다. true 로 설정하면 그때부터 탐색하는
	 * 노드에 대해서는 innerHTML 에 _cssquery_UID 가 나오지 않도록 하지만 탐색속도는 다소 느려질 수 있다.
	 * 
	 * @param {Boolean}
	 *            flag true 로 셋팅하면 _cssquery_UID 가 나오지 않음
	...
	/**
	 * 
	 * querySelector가 있는 브라우져에서 extreme을 실행시키면 querySelector을 사용할수 있는 커버리지가 높아져
	 * 전체적으로 속도가 빨리진다. 하지만 ID가 없는 엘리먼트를 기준 엘리먼트로 넣었을 때 기준 엘리먼트에 임의의 아이디가 들어간다.
	 * 
	 * @param {Boolean}
	 *            bExtreme true
	 * 
 ...
 *         object는 브라우저 이름과 버전을 속성으로 가진다. 브라우저 이름은 영어 소문자로 표시하며, 사용자의 브라우저와 일치하는
 *         브라우저 이름은 true를 가진다.
 * @since 1.4.3 부터 mobile,msafari,mopera,mie 사용 가능.
 * @since 1.4.5 부터 ipad에서 mobile은 false를 반환 한다.
 * @example oAgent = $Agent().navigator(); // 사용자가 파이어폭스 3를 사용한다고 가정한다.
 *          oAgent.camino // false oAgent.firefox // true oAgent.konqueror //
 *          false oAgent.mozilla //true oAgent.netscape // false oAgent.omniweb
 *          //false oAgent.opera //false oAgent.webkit /false oAgent.safari
 *          //false oAgent.ie //false oAgent.chrome //false oAgent.icab //false
 *          oAgent.version //3 oAgent.nativeVersion //-1 (1.4.2부터 사용 가능, IE8에서
 *          호환 모드 사용시 nativeVersion은 8로 나옴.) oAgent.getName() // firefox
 * 
 */
 ...
 * 
 * @return {Object} 운영체제 정보 객체. 운영체제의 영문 이름을 속성으로 가지며, 사용자가 사용하는 운영체제와 동일한 이름의
 *         속성은 true를 가진다.
 * @since 1.4.3 부터 iphone,android,nokia,webos,blackberry,mwin 사용 가능.
 * @since 1.4.5 부터 ipad 사용가능.
 * @example oOS = $Agent().os(); // 사용자의 운영체제가 Windows XP라고 가정한다. oOS.linux //
 *          false oOS.mac // false oOS.vista // false oOS.win // true
 *          oOS.win2000 // false oOS.winxp // true oOS.xpsp2 // false oOS.win7 //
 *          false oOS.getName() // winxp
 * 
 */
 ...
 *         object.installed는 플래시 플레이어 설치 여부를 boolean 값으로 가지고 object.version은 플래시
 *         플레이어의 버전을 가진다. 플래시 버전을 탐지하지 못하면 flash.version은 -1의 값을 가진다.
 * @example var oFlash = $Agent.flash(); oFlash.installed // 플래시 플레이어를 설치했다면
 *          true oFlash.version // 플래시 플레이어의 버전.
 * 
 */
 ...
 *          object.installed은 실버라이트 플레이어 설치 여부를 boolean 값으로 가지고 object.version은
 *          실버라이트 플레이어의 버전을 가진다. 플레이어의 버전을 탐지하지 못하면 object.version의 값은 -1이 된다.
 * @example var oSilver = $Agent.silverlight(); oSilver.installed // Silverlight
 *          플레이어를 설치했다면 true oSilver.version // Silverlight 플레이어의 버전.
 * 
 */
 ...
 * @param {Array|$A}
 *            array 배열. 만약 매개 변수를 생략하면 빈 배열을 가진 새로운 $A 객체를 리턴한다.
 * @constructor
 * @description [Lite]
 * @author Kim, Taegon
 * 
 * @example var zoo = ["zebra", "giraffe", "bear", "monkey"]; var waZoo =
 *          $A(zoo); // ["zebra", "giraffe", "bear", "monkey"]를 래핑한 $A 객체를 생성하여
 *          반환
 * 
 */
 ...
 * @description [Lite]
 * 
 * @example var zoo = ["zebra", "giraffe", "bear", "monkey"];
 *          $A(zoo).toString(); // 결과 : zebra,giraffe,bear,monkey
 * 
 */
 ...
 * 
 * @param {Number}
 *            nIndex 조회할 배열의 인덱스. 인덱스는 0부터 시작한다.
 ...
 * @example var zoo = ["zebra", "giraffe", "bear", "monkey"]; var waZoo =
 *          $A(zoo);
 *  // 원소 값 조회 waZoo.get(1); // 결과 : giraffe waZoo.get(3); // 결과 :
 * monkey
 * 
 */
 ...
 * @param {Value}
 *            [oValue] 새로운 원소를 추가할 때 사용할 초기값
 * @return {Number|$A} 매개 변수를 모두 생략하면 현재 내부 배열의 크기를 리턴하고,<br>
 *         매개 변수를 지정한 경우에는 내부 배열을 변경한 $A 객체를 리턴한다.
 * 
 * @example var zoo = ["zebra", "giraffe", "bear", "monkey"]; var birds =
 *          ["parrot", "sparrow", "dove"];
 *  // 배열의 크기 조회 $A(zoo).length(); // 결과 : 4
 *  // 배열의 크기 지정 (원소가 삭제되는 경우) $A(zoo).length(2); // 결과 : ["zebra", "giraffe"]
 *  // 배열의 크기 지정 (원소가 추가되는 경우) $A(zoo).length(6, "(Empty)"); // 결과 : ["zebra",
 * "giraffe", "bear", "monkey", "(Empty)", "(Empty)"]
 * 
 * $A(zoo).length(5, birds); // 결과 : ["zebra", "giraffe", "bear", "monkey",
 * ["parrot", "sparrow", "dove"]]
 * 
 */
 ...
 * @description [Lite]
 * 
 * @example var arr = $A([1,2,3]);
 *  // 값 검색 arr.has(3); // 결과 : true arr.has(4); // 결과 : false
 * 
 */
 ...
 * @description [Lite]
 * 
 * @example var zoo = ["zebra", "giraffe", "bear"]; va r waZoo = $A(zoo);
 *  // 값 검색 후 인덱스 리턴 waZoo.indexOf("giraffe"); // 1
 ...
 * @description [Lite]
 * 
 * @example var waNum = $A([1, 2, 3]); waNum.$value(); // 원래의 배열인 [1, 2, 3]이
 *          반환된다
 * 
 */
 ...
 * 
 * @param {oValue1,
 *            ..., oValueN} oValueN 추가할 N 개의 값
 * @return {Number} 하나 이상의 원소를 추가한 내부 배열의 크기
 * @description [Lite]
 * 
 * @example var arr = $A([1,2,3]);
 *  // 원소 추가 arr.push(4); // 결과 : 4 반환, 내부 배열은 [1,2,3,4]로 변경 됨
 ...
 * @see $A#unshift
 * @description [Lite]
 * @example var arr = $A(['Melon','Grape','Apple','Kiwi']);
 * 
 ...
 * 
 * @param {oValue1,
 *            ..., oValueN} oValueN 삽입할 하나 이상의 값
 * @return {Number} 원소를 추가한 후의 배열의 크기
 * @description [Lite]
 * @example var arr = $A([4,5]);
 * 
 * arr.unshift('c'); // 결과 : 3 반환, 내부 배열은 ["c", 4, 5]로 변경 됨. arr.unshift('a',
 * 'b'); // 결과 : 5 반환, 내부 배열은 ["a", "b", "c", 4, 5]로 변경 됨.
 * 
 */
 ...
 * @param {Object}
 *            [oThis] 콜백 함수가 객체의 메서드일 때 콜백 함수 내부에서 사용할 this
 * @return {$A} $A 객체
 ...
 * @description [Lite]
 * 
 * @example var waZoo = $A(["zebra", "giraffe", "bear", "monkey"]);
 * 
 * waZoo.forEach(function(value, index, array) { document.writeln((index+1) + ". " +
 * value); });
 *  // 결과 : // 1. zebra // 2. giraffe // 3. bear // 4. monkey
 * 
 * @example var waArray = $A([1, 2, 3]);
 * 
 * waArray.forEach(function(value, index, array) { array[index] += 10; });
 * 
 * document.write(waArray.$value()); // 결과 : 11, 12, 13 (내부 배열에 10씩 더해짐)
 * 
 */
 ...
 * 
 * @param {Number}
 *            nStart 잘라낼 부분의 시작 인덱스. 인덱스는 0부터 시작한다.
 * @param {Number}
 *            nEnd 잘라낼 부분의 바로 뒤 인덱스
 * @return {$A} 내부 배열의 일부를 추출한 새로운 $A 객체.<br>
 *         nStart 값이 0 보다 작거나 혹은 nStart 값이 nEnd 값 보다 크거나 같으면 빈 배열을 가지는 $A 객체를
 *         리턴한다.
 * @description [Lite]
 * 
 * @example var arr = $A([12, 5, 8, 130, 44]); var newArr = arr.slice(1,3); //
 *          잘라낸 배열인 [5, 8]를 래핑한 $A 객체를 리턴한다. (원래의 배열은 변화 없음)
 * 
 * @example var arr = $A([12, 5, 8, 130, 44]); var newArr = arr.slice(3,3); //
 *          []를 래핑한 $A 객체를 리턴한다.
 * 
 */
 ...
 * 
 * @param {Number}
 *            nIndex 삭제할 부분의 시작 인덱스. 인덱스는 0부터 시작한다.
 * @param {Number}
 *            [nHowMany] 삭제할 원소의 개수.<br>
 *            이 값과 oValueN 를 생략하면 nIndex 번째 원소부터 배열의 마지막 원소까지 삭제한다.<br>
 *            이 값을 0 혹은 지정하지 않고 oValueN 에 값을 지정하면 nIndex 번째 위치에 oValueN 값이 추가된다.
 * @param {Value1,
 *            ...,ValueN} [oValueN] 삭제한 배열에 추가할 하나 이상의 값. nIndex 값의 인덱스부터 추가된다.
 * @returns {$A} 삭제한 원소를 래핑하는 새로운 $A 객체
 * @description [Lite]
 * 
 * @example var arr = $A(["angel", "clown", "mandarin", "surgeon"]);
 * 
 * var removed = arr.splice(2, 0, "drum"); // arr의 내부 배열은 ["angel", "clown",
 * "drum", "mandarin", "surgeon"]로 인덱스 2에 drum이 추가 됨 // removed의 내부 배열은 []로 삭제된
 * 원소가 없음
 * 
 * removed = arr.splice(3, 1); // arr의 내부 배열은 ["angel", "clown", "drum",
 * "surgeon"]로 mandarin이 삭제 됨 // removed의 내부 배열은 삭제된 원소 ["mandarin"]를 가짐
 * 
 * removed = arr.splice(2, 1, "trumpet", "parrot"); // arr의 내부 배열은 ["angel",
 * "clown", "trumpet", "parrot", "surgeon"]로 drum이 삭제되고 새로운 원소가 추가 됨 // removed의
 * 내부 배열은 삭제된 원소 ["drum"]을 가짐
 * 
 * removed = arr.splice(3); // arr의 내부 배열은 ["angel", "clown", "trumpet"]로 인덱스
 * 3부터 마지막 원소가 삭제되었음 // removed의 내부 배열은 삭제된 원소 ["parrot", "surgeon"]을 가짐
 * 
 */
 ...
 * @description [Lite]
 * 
 * @example var dice = $A([1,2,3,4,5,6]);
 * 
 * dice.shuffle(); document.write("You get the number " + dice.get(0)); // 결과 :
 * 1부터 6까지의 숫자 중 랜덤한 숫자
 * 
 */
 ...
 * 
 * @remark 내부적으로는 강제로 예외를 발생시키는 구조이므로, try ~ catch 영역에서 이 메소드를 실행하면 정상적으로 동작하지
 *         않을 수 있다.
 * 
 * @description [Lite]
 * @see $A#Continue
 ...
 * @see $A#filter
 * @see $A#map
 * @example $A([1,2,3,4,5]).forEach(function(value,index,array) { // 값이 4보다 크면
 *          종료 if (value > 4) $A.Break(); ... });
 * 
 */
 ...
 * @see $A#filter
 * @see $A#map
 * @example $A([1,2,3,4,5]).forEach(function(value,index,array) { // 값이 짝수면 처리를
 *          하지 않음 if (value%2 == 0) $A.Continue(); ... });
 * 
 */
 ...
 * @example var waZoo = $A(["zebra", "giraffe", "bear", "monkey"]);
 * 
 * waZoo.map(function(value, index, array) { return (index+1) + ". " + value;
 * }); // 결과 : [1. zebra, 2. giraffe, 3. bear, 4. monkey]
 * 
 * @example var waArray = $A([1, 2, 3]);
 * 
 * waArray.map(function(value, index, array) { return value + 10; });
 * 
 * document.write(waArray.$value()); // 결과 : 11, 12, 13 (내부 배열에 10씩 더해짐)
 * 
 */
...
/**
 * 
 * 배열의 모든 원소를 순회하면서 콜백 함수를 실행한다. 실행이 끝나면 filter 메서드는 콜박 함수를 만족하는 원소로 이루어진 새로운 $A
 * 객체를 반환한다.
 * 
 * @param {Function}
 *            fCallback 순회하면서 실행할 콜백 함수.<br>
 *            <br>
 *            콜백 함수는 fCallback(value, index, array)의 형식으로 작성해야 한다. 여기서 value 는
 *            배열이 가진 원소의 값, index 는 해당 원소의 인덱스, array 는 원본 배열이다.<br>
 *            <br>
 ...
 * 
 * @example var arr = $A([1,2,3,4,5]);
 *  // 필터링 함수 function filterFunc(value, index, array) { if (value > 2) {
...
/**
 * 
 * 배열의 모든 원소를 순회하면서 콜백 함수를 실행한다. 동시에 배열의 모든 원소가 콜백 함수를 만족하는지(콜백 함수가 true를 리턴하는지)
 * 검사한다. <br>
 ...
 * @see $A#some
 * 
 * @example function isBigEnough(value, index, array) { return (value >= 10); }
 * 
 * var try1 = $A([12, 5, 8, 130, 44]).every(isBigEnough); // 결과 : false var try2 =
 * $A([12, 54, 18, 130, 44]).every(isBigEnough); // 결과 : true
 * 
 */
 ...
 *            <br>
 *            콜백 함수는 fCallback(value, index, array) 의 형식으로 작성해야 한다. 여기서 value 는
 *            배열이 가진 원소, index 는 해당 원소의 인덱스, array 는 원본 배열이다.<br>
 *            <br>
 *            콜백 함수는 Boolean 을 리턴해야한다.<br>
 * 
 * @param {Object}
 ...
 * @see $A#every
 * 
 * @example function twoDigitNumber(value, index, array) { return (value >= 10 &&
 *          value < 100); }
 * 
 * var try1 = $A([12, 5, 8, 130, 44]).some(twoDigitNumber); // 결과 : true var
 * try2 = $A([1, 5, 8, 130, 4]).some(twoDigitNumber); // 결과 : false
 * 
 */
 ...
 * 
 * @param {Value,
 *            ..., ValueN} oValueN 배열에서 제외할 값
 * @return {$A} 배열에서 특정 값을 제외한 새로운 $A 객체
 * 
 * @example var arr = $A([12, 5, 8, 130, 44]);
 * 
 * var newArr1 = arr.refuse(12);
 * 
 * document.write(arr); // 결과 : [12, 5, 8, 130, 44] document.write(newArr1); //
 * 결과 : [5, 8, 130, 44]
 * 
 * var newArr2 = newArr1.refuse(8, 44, 130);
 * 
 * document.write(newArr1); // 결과 : [5, 8, 130, 44] document.write(newArr2); //
 * 결과 : [5]
 * 
 */
 ...
 * 
 * @example var arr = $A([10, 3, 76, 5, 4, 3]);
 * 
...
/**
 * 
 * $Ajax는 서버와 브라우저 사이의 비동기 통신, 즉 Ajax 통신을 지원한다. $Ajax는 XHR(XMLHTTPRequest)을 사용한
 * 기본적인 방식과 함께 다른 호스트(Host) 사이의 통신을 위한 여러 방식을 제공한다. $Ajax 객체의 기본적인 초기화 방식은 다음과
 * 같다. <textarea name="code" class="js:nocontrols"> var oAjax = new
 * $Ajax('server.php', { type : 'xhr', method : 'get', // GET 방식으로 통신 onload :
 * function(res){ // 요청이 완료되면 실행될 콜백 함수 $('list').innerHTML = res.text(); },
 * timeout : 3, // 3초 이내에 요청이 완료되지 않으면 ontimeout 실행 (생략 시 0) ontimeout :
 * function(){ // 타임 아웃이 발생하면 실행될 콜백 함수, 생략 시 타임 아웃이 되면 아무 처리도 하지 않음
 * alert("Timeout!"); }, async : true // 비동기로 호출하는 경우, 생략하면 true });
 * oAjax.request(); } </textarea>
 * 
 * @extends core
 ...
 * 
 * @param {String}
 *            url Ajax 요청을 보낼 서버 측 URL.<br>
 * @param {Object}
 *            option $Ajax 에서 사용하는 콜백 함수, 통신 방식 등과 같은 다양한 정보를 정의한다.<br>
 *            <br>
 *            option 객체의 프로퍼티와 사용법은 아래에서 설명한다.<br>
 *            <table> <thead style="background-color:#D2E0E6;">
 *            <th>프로퍼티 명</th>
 *            <th>타입</th>
 *            <th>설명</th>
 *            </thead> <tbody>
 ...
 *            <td>String</td>
 *            <td> Ajax 구현 방식. 생략 시 기본 값은 "xhr"
 *            <ul>
 *            <li><strong>xhr</strong> 브라우저에 내장된 XMLHttpRequest 객체를 이용하여 Ajax
 *            요청을 처리한다.<br>
 *            응답으로는 text, xml, json 방식 모두 사용이 가능하며, 요청 실패 시 HTTP 응답코드를 통해 원인 파악이
 *            가능하다.<br>
 *            단, Cross-Domain 에서는 사용할 수 없다. </li>
 *            <li><strong>iframe</strong> iframe 을 프록시로 사용하여 Ajax 요청을 처리한다.
 *            Cross-Domain 에서 사용한다.<br>
 *            로컬(요청 하는 쪽)과 원격(요청 받는 쪽)에 모두 프록시용 HTML 파일을 만들어<br>
 *            iframe 에서 원격 프록시에 요청하면, 원격 프록시에서 원격 도메인의 페이지에 XHR 로 Ajax 요청을 한다.<br>
 *            응답을 받은 원격 프록시에서 로컬 프록시로 응답을 전달하면 로컬 프록시에서 최종적으로 콜백 함수(onload)를
 *            호출하여 처리된다.<br>
 *            <ul type="disc">
 *            <li>원격 프록시 파일 : ajax_remote_callback.html</li>
 *            <li>로컬 프록시 파일 : ajax_local_callback.html</li>
 *            </ul> ※ IE 에서는 "딱.딱." 하는 페이지 이동음이 들릴 수도 있다 (요청당 2회). </li>
 *            <li><strong>jsonp</strong> JSON 과 &lt;script&gt; 태그를 사용하여 사용하여
 *            Ajax 요청을 처리한다. Cross-Domain 에서 사용한다.<br>
 *            &lt;script&gt; 태그를 동적으로 생성하여, 요청할 원격 페이지를 스크립트에 삽입하여 GET 방식으로 요청을
 *            전송한다.<br>
 *            요청 시에 콜백 함수를 매개 변수로 넘기면, 원격 페이지에서 전달받은 콜백 함수명으로 아래와 같이 응답을 보낸다.<br>
 *            <ul type="disc">
 *            <li>function_name(...결과 값...)</li>
 *            </ul>
 *            응답은 콜백 함수(onload)에서 처리된다.<br> ※ GET 방식만 가능하므로, 전송 데이터 길이는 URL 에
 *            허용하는 길이로 제한된다. </li>
 *            <li><strong>flash</strong> 플래시 객체를 사용하여 Ajax 요청을 처리한다.
 *            Cross-Domain 에서 사용한다.<br>
 ...
 *            $Ajax.SWFRequest.write 메서드 사용하여 초기화하며 해당 메서드는 &lt;body&gt; 태그 내에
 *            작성한다. </li>
 ...
 *            <td>Number</td>
 *            <td> 요청 타임아웃 시간. 생략 시 0 (단위는 초).<br>
 *            타임아웃 시간 내에 요청이 완료되지 않으면 중지시킨다.<br>
 *            비동기 호출인 경우에만 사용 가능하다. </td>
 *            </tr>
 *            <tr>
 *            <td style="font-weight:bold;">onload</td>
 *            <td>Function</td>
 *            <td> 요청이 완료되면 실행할 콜백 함수. 반드시 지정해야 한다.<br>
 *            매개 변수로 응답 객체인 $Ajax.Response 객체가 전달 된다. </td>
 *            </tr>
 *            <tr>
 *            <td style="font-weight:bold;">onerror</td>
 *            <td>Function</td>
 *            <td> 요청이 실패하면 실행할 콜백 함수.<br>
 *            생략하면 에러가 발생해도 onload 를 실행한다. </td>
 *            </tr>
 *            <tr>
 *            <td style="font-weight:bold;">ontimeout</td>
 *            <td>Function</td>
 *            <td> 타임아웃이 되었을 때 실행할 콜백 함수.<br>
 *            생략하면 타임아웃 발생 후에 아무런 처리를 하지 않는다. </td>
 *            </tr>
 *            <tr>
 *            <td style="font-weight:bold;">proxy</td>
 *            <td>String</td>
 *            <td> 로컬 프록시 파일(ajax_local_callback.html)의 경로.<br>
 *            type 이 "iframe" 일 때 사용하며 반드시 지정해야 한다. </td>
 *            </tr>
 *            <tr>
 *            <td style="font-weight:bold;">jsonp_charset</td>
 *            <td>String</td>
 *            <td> 요청 시 사용할 &lt;script&gt; 인코딩 방식.<br>
 *            type 이 "jsonp" 일 때 사용한다. 생략하면 "utf-8" 이 기본값이다. (0.4.2 부터 지원) </td>
 *            </tr>
 *            <tr>
 *            <td style="font-weight:bold;">callbackid</td>
 *            <td>String</td>
 *            <td> 콜백 함수 이름에 사용할 아이디 값.<br>
 *            type 이 "jsonp" 일 때 사용한다. (1.3.0 부터 지원)<br>
 *            생략하면 자동으로 랜덤한 아이디 값을 생성하여 사용한다. <br>
 *            jsonp 방식에서 Ajax 요청 시, 콜백 함수 이름에 랜덤한 아이디 값을 덧붙여 만든 콜백 함수 이름을 서버로
 *            전달한다.<br>
 *            이 때 랜덤한 값을 아이디로 사용하여 넘기므로 요청 URL이 매번 새롭게 생성되어 캐쉬 서버가 아닌 서버로 직접
 *            데이터를 요청하게 된다.<br>
 *            아이디 값을 지정하면 랜덤한 아이디 값으로 콜백 함수 이름을 생성하지 않으므로<br>
 *            캐쉬 서버를 사용하여 그에 대한 히트율을 높이고자 할 때 아이디를 지정하여 사용할 수 있다. </td>
 *            </tr>
 *            <tr>
 *            <td style="font-weight:bold;">callbackname</td>
 *            <td>String</td>
 *            <td> 콜백 함수 이름을 가지는 매개변수 이름.<br>
 *            type 이 "jsonp" 일 때 사용한다. 기본 값은 "_callback" 이다. (1.3.8 부터 지원) </td>
 *            </tr>
 *            <tr>
 *            <td style="font-weight:bold;">sendheader</td>
 *            <td>Boolean</td>
 *            <td> 요청 헤더를 전송할지 여부.<br>
 ...
 *            기본 값은 true 이다. (1.3.4부터 지원) </td>
 *            </tr>
 *            <tr>
 *            <td style="font-weight:bold;">async</td>
 *            <td>Boolean</td>
 *            <td> 비동기 호출 여부.<br>
 *            type 이 "xhr" 일 때만 유효하다. 기본 값은 true 이다. (1.3.7부터 지원) </td>
 *            </tr>
 *            <tr>
 *            <td style="font-weight:bold;">decode</td>
 *            <td>Boolean</td>
 *            <td> type 이 "flash" 일 때 사용하며, 요청한 데이터 안에 utf-8 이 아닌 다른 인코딩이 되어 있을때
 *            false 로 지정한다.<br>
 *            기본 값은 true 이다. (1.4.0부터 지원) </td>
 *            </tr>
 *            <tr>
 *            <td style="font-weight:bold;">postBody</td>
 *            <td>Boolean</td>
 *            <td> 요청 시 서버로 전달할 데이터를 Body 영역에 전달할 지의 여부.<br>
 *            type 이 "xhr" 이고 method 가 "get"이 아니어야 유효하며 REST 환경에서 사용된다.<br>
 *            기본값은 false 이다. (1.4.2부터 지원) </td>
 *            </tr>
 *            </tbody> </table>
 * 
 * @constructor
 * @description [Lite]
 * @see <a
 *      href="http://dev.naver.com/projects/jindo/wiki/cross%20domain%20ajax">Cross
 *      Domain Ajax 이해</a>
 * @author Kim, Taegon
 * 
 * @example // 'Get List' 버튼 클릭 시, 서버에서 데이터를 받아와 리스트를 구성하는 예제 // (1) 서버 페이지와 서비스
 *          페이지가 같은 도메인에 있는 경우 - xhr
 *  // [client.html] <!DOCTYPE html> <html> <head> <title>Ajax Sample</title>
 * <meta http-equiv="content-type" content="text/html; charset=utf-8"/> <script
 * type="text/javascript" language="javascript" src="lib/jindo.all.js"></script>
 * <script type="text/javascript" language="javascript"> function getList() {
 * var oAjax = new $Ajax('server.php', { type : 'xhr', method : 'get', // GET
 * 방식으로 통신 onload : function(res){ // 요청이 완료되면 실행될 콜백 함수 $('list').innerHTML =
 * res.text(); }, timeout : 3, // 3초 이내에 요청이 완료되지 않으면 ontimeout 실행 (생략 시 0)
 * ontimeout : function(){ // 타임 아웃이 발생하면 실행될 콜백 함수, 생략 시 타임 아웃이 되면 아무 처리도 하지 않음
 * alert("Timeout!"); }, async : true // 비동기로 호출하는 경우, 생략하면 true });
 * oAjax.request(); } </script> </head> <body> <button onclick="getList();
 * return false;">Get List</button>
 * 
 * <ul id="list">
 * 
 * </ul>
 ...
 * <li>세번째</li>"; ?>
 * 
 * @example // 'Get List' 버튼 클릭 시, 서버에서 데이터를 받아와 리스트를 구성하는 예제 // (2) 서버 페이지와 서비스
 *          페이지가 같은 도메인에 있는 경우 - iframe
 *  // [http://local.com/some/client.html] <!DOCTYPE html> <html>
 * <head> <title>Ajax Sample</title> <meta http-equiv="content-type"
 * content="text/html; charset=utf-8"/> <script type="text/javascript"
 * language="javascript" src="lib/jindo.all.js"></script> <script
 * type="text/javascript" language="javascript"> function getList() { var oAjax =
 * new $Ajax('http://server.com/some/some.php', { type : 'iframe', method :
 * 'get', // GET 방식으로 통신 // POST로 지정하면 원격 프록시 파일에서 some.php 로 요청 시에 POST 방식으로 처리
 * onload : function(res){ // 요청이 완료되면 실행될 콜백 함수 $('list').innerHTML =
 * res.text(); }, // 로컬 프록시 파일의 경로. // 반드시 정확한 경로를 지정해야 하며, 로컬 도메인의 경로라면 어디에 두어도
 * 상관 없음 // (※ 원격 프록시 파일은 반드시 원격 도메인 서버의 도메인 루트 상에 두어야 함) proxy :
 * 'http://local.naver.com/some/ajax_local_callback.html' }); oAjax.request(); }
 * 
 * </script> </head> <body> <button onclick="getList(); return false;">Get List</button>
 * 
 * <ul id="list">
 * 
 * </ul>
 * </body> </html>
 *  // [http://server.com/some/some.php] <?php echo "
 * <li>첫번째</li>
 * <li>두번째</li>
 * <li>세번째</li>"; ?>
 * 
 * @example // 'Get List' 버튼 클릭 시, 서버에서 데이터를 받아와 리스트를 구성하는 예제 // (3) 서버 페이지와 서비스
 *          페이지가 같은 도메인에 있는 경우 - jsonp
 *  // [http://local.com/some/client.html] <!DOCTYPE html> <html>
 * <head> <title>Ajax Sample</title> <meta http-equiv="content-type"
 * content="text/html; charset=utf-8"/> <script type="text/javascript"
 * language="javascript" src="lib/jindo.all.js"></script> <script
 * type="text/javascript" language="javascript"> function getList(){ var oAjax =
 * new $Ajax('http://server.com/some/some.php', { type: 'jsonp', method: 'get', //
 * type 이 jsonp 이면 get 으로 지정하지 않아도 자동으로 get 으로 처리함 (생략가능) jsonp_charset:
 * 'utf-8', // 요청 시 사용할 <script> 인코딩 방식 (생략 시 utf-8) onload: function(res){ //
 * 요청이 완료되면 실행될 콜백 함수 var response = res.json(); var welList =
 * $Element('list').empty();
 * 
 * for (var i = 0, nLen = response.length; i < nLen; i++) { welList.append($("
 * <li>" + response[i] + "</li>
 * ")); } }, callbackid: '12345', // 콜백 함수 이름에 사용할 아이디 값 (생략가능) callbackname:
 * 'ajax_callback_fn' // 서버에서 사용할 콜백 함수이름을 가지는 매개 변수 이름 (생략 시 '_callback') });
 * oAjax.request(); } </script> </head> <body> <button onclick="getList();
 * return false;">Get List</button>
 * 
 * <ul id="list">
 ...
 * </ul>
 * </body> </html>
 *  // [http://server.com/some/some.php] <?php $callbackName =
 * $_GET['ajax_callback_fn']; echo $callbackName."(['첫번째','두번째','세번째'])"; ?>
 * 
 * @example // 'Get List' 버튼 클릭 시, 서버에서 데이터를 받아와 리스트를 구성하는 예제 // (4) 서버 페이지와 서비스
 *          페이지가 같은 도메인에 있는 경우 - flash
 *  // [http://local.com/some/client.html] <!DOCTYPE html> <html>
 * <head> <title>Ajax Sample</title> <meta http-equiv="content-type"
 * content="text/html; charset=utf-8"/> <script type="text/javascript"
 * language="javascript" src="lib/jindo.all.js"></script> <script
 * type="text/javascript" language="javascript"> function getList(){ var oAjax =
 * new $Ajax('http://server.com/some/some.php', { type : 'flash', method :
 * 'get', // GET 방식으로 통신 sendheader : false, // 요청 헤더를 전송할지 여부. (생략 시 true)
 * decode : true, // 요청한 데이터 안에 utf-8 이 아닌 다른 인코딩이 되어 있을때 false. (생략 시 true)
 * onload : function(res){ // 요청이 완료되면 실행될 콜백 함수 $('list').innerHTML =
 * res.text(); }, }); oAjax.request(); } </script> </head> <body> <script
 * type="text/javascript"> $Ajax.SWFRequest.write("swf/ajax.swf"); // Ajax 호출을
 * 하기 전에 반드시 플래시 객체를 초기화 </script> <button onclick="getList(); return
 * false;">Get List</button>
 * 
 * <ul id="list">
 * 
 * </ul>
 ...
 * 
 * @remark 요청 타입(type)이 "flash" 이면 이 메소드를 실행하기 전에 body 태그 내부에서
 *         $Ajax.SWFRequest.write() 명령어를 반드시 실행해야 한다.
 * 
 * @param {Object}
 *            oData 서버로 전송할 데이터.
 * @return {$Ajax} $Ajax 객체
 ...
 * 
 * 
 * var ajax = $Ajax("http://www.remote.com", { onload : function(res) { //
 * onload 핸들러 } });
 * 
 ...
 * @description [Lite]
 * @example var ajax = $Ajax("http://www.remote.com",{ onload : function(res){ //
 *          onload 핸들러 } });
 * 
...
/**
 * 
 * abort 메서드는 서버로 전송한 Ajax 요청을 취소한다. Ajax 요청의 응답 시간이 길거나 강제로 Ajax 요청을 취소할 경우
 * 사용한다.
 * 
 * @return {$Ajax} 전송을 취소한 $Ajax 객체
 * @description [Lite]
 * @example var ajax = $Ajax("http://www.remote.com", { timeout : 3, ontimeout :
 *          function() { stopRequest(); } onload : function(res) { // onload 핸들러 }
 *          }).request( {key1:"value1", key2:"value2"} );
 * 
 * function stopRequest() { ajax.abort(); }
 * 
 */
 ...
 *            <br>
 *            객체인 경우에는 속성 이름으로 정보를 찾아 속성의 값으로 설정한다. 객체에 두 개 이상의 속성을 지정하면 여러 속성
 *            값을 한 번에 설정할 수 있다.
 * @param {String}
 *            [value] 새로 설정할 정보의 값. name 매개 변수가 문자열인 경우에만 사용된다.
 * @return {String|$Ajax} 정보의 값을 가져올 때는 문자열을, 값을 설정할 때는 $Ajax 객체를 리턴한다.
 * @description [Lite]
 * @example var ajax = $Ajax("http://www.remote.com", { type : "xhr", method :
 *          "get", onload : function(res) { // onload 핸들러 } });
 * 
 * var request_type = ajax.option("type"); // type 인 xhr 을 리턴한다.
 * ajax.option("method", "post"); // method 를 post 로 설정한다. ajax.option( {
 * timeout : 0, onload : handler_func } ); // timeout 을 으로, onload 를
 * handler_func 로 설정한다.
 * 
 */
 ...
 * 헤더를 설정하려면 헤더의 이름과 값을 각각 매개 변수로 사용하거나, 헤더의 이름과 값을 원소로 가지는 객체를 매개 변수로 사용한다. 만약
 * 여러 개의 속성을 가진 객체를 사용하면 두 개 이상의 헤더를 한 번에 설정할 수 있다.<br>
 ...
 *            <br>
 *            객체인 경우에는 속성 이름으로 정보를 찾아 속성의 값으로 설정한다. 객체에 두 개 이상의 속성을 지정하면 HTTP 요청
 *            헤더에서 여러 속성 값을 한 번에 설정할 수 있다.
 * @param {Value}
 *            [value] 설정할 헤더 값. name 매개 변수가 문자열인 경우에만 사용된다.
 * @return {String|$Ajax} 정보의 값을 가져올 때는 문자열을, 값을 설정할 때는 $Ajax 객체를 리턴한다.
 * @description [Lite]
 * @example var customheader = ajax.header("myHeader"); // HTTP 요청 헤더에서 myHeader
 *          의 값 ajax.header( "myHeader", "someValue" ); // HTTP 요청 헤더의 myHeader
 *          를 someValue 로 설정한다. ajax.header( { anotherHeader : "someValue2" } ); //
 *          HTTP 요청 헤더의 anotherHeader 를 someValue2 로 설정한다.
 * 
 */
 ...
 *        $Ajax 객체에서 request 요청 처리 완료 후, 생성되며 $Ajax 생성 시에 설정한 onload 콜백 함수의 매개
 *        변수로 전달된다.
 * @constructor
 * @param {Object}
 *            req 요청 객체
 ...
 * @description [Lite]
 * 
 * @example // some.xml ﻿<?xml version="1.0" encoding="utf-8"?> <data>
 *          <li>첫번째</li>
 *          <li>두번째</li>
 *          <li>세번째</li>
 *          </data>
 *  // client.html var oAjax = new $Ajax('some.xml', { type : 'xhr',
 * method : 'get', onload : function(res){ var elData =
 * cssquery.getSingle('data', res.xml()); // 응답을 XML 객체로 리턴한다
 * $('list').innerHTML = elData.firstChild.nodeValue; }, }).request();
 * 
 */
 ...
 * @description [Lite]
 * 
 * @example // some.php <?php echo "
 *          <li>첫번째</li>
 *          <li>두번째</li>
 *          <li>세번째</li>"; ?>
 *  // client.html var oAjax = new $Ajax('some.xml', { type : 'xhr',
 * method : 'get', onload : function(res){ $('list').innerHTML = res.text(); //
 * 응답을 문자열로 리턴한다 }, }).request();
 * 
 */
 ...
 * @description [Lite]
 * 
 * @example var oAjax = new $Ajax('some.php', { type : 'xhr', method : 'get',
 *          onload : function(res){ if(res.status() == 200){ // HTTP 응답 코드를
 *          확인한다. $('list').innerHTML = res.text(); } }, }).request();
 * 
 */
 ...
 * 
 * @example var oAjax = new $Ajax('some.php', { type : 'xhr', method : 'get',
 *          onload : function(res){ if(res.readyState() == 4){ // 응답의 readyState
 *          를 확인한다. $('list').innerHTML = res.text(); } }, }).request();
 * 
 */
 ...
 * @description [Lite]
 * 
 * @example // some.php <?php echo "['첫번째', '두번째', '세번째']"; ?>
 *  // client.html var oAjax = new $Ajax('some.php', { type : 'xhr',
 * method : 'get', onload : function(res){ var welList =
 * $Element('list').empty(); var jsonData = res.json(); // 응답을 JSON 객체로 리턴한다
 * 
 * for(var i = 0, nLen = jsonData.length; i < nLen; i++){ welList.append($("
 * <li>" + jsonData[i] + "</li>
 * ")); } }, }).request();
 * 
 */
 ...
 * 
 * @example var oAjax = new $Ajax('some.php', { type : 'xhr', method : 'get',
 *          onload : function(res){ res.header(); // 응답 헤더 전체를 리턴한다.
 ...
 * @class Ajax 요청 타입(type)이 jsonp 인 요청 객체를 생성하며, jindo.$Ajax 생성자에서 요청 객체 생성 시
 *        사용한다.
 * 
 */
 ...
 * 
 * @extends $Ajax.RequestBase
 * @class Ajax 요청 타입(type)이 flash 인 요청 객체를 생성하며, jindo.$Ajax 생성자에서 요청 객체 생성 시
 *        사용한다.
 * 
 */
						...
						if (this.status == 200) {
							if (typeof data == "string") {
								try {
									...
									/*
									 * 
									 * 데이터 안에 utf-8이 아닌 다른 인코딩일때 디코딩을 안하고 바로
									 * text에 저장.
									 * 
									 */
 ...
 * 
 * @param {String}
 *            [swf_path] 통신을 담당할 플래시 파일의 경로. 생략하면 기본 값은 "./ajax.swf" 이다.
 * 
...
jindo.$Ajax._checkFlashLoad = function() {
	jindo.$Ajax._checkFlashKey = setTimeout(function() {
		...
		// [SMARTEDITORSUS-334] Flash 를 지원하지 않는 환경(ex. iOS)에서 경고창을 띄우지 않도록 임시적으로
		// 주석 처리함
 ...
 * 
 * @extends $Ajax.RequestBase
 * @class Ajax 요청 타입(type)이 iframe 인 요청 객체를 생성하며, jindo.$Ajax 생성자에서 요청 객체 생성 시
 *        사용한다.
 * 
 */
 ...
 * @param {Object}
 *            option $Ajax.Queue 에서 서버에 요청할 때 사용하는 다양한 정보를 정의한다.
 *            <ul>
 *            <li> async : 비동기/동기 요청 여부를 설정한다. 비동기인 경우 true 이며, 기본 값은 false 이다.
 *            </li>
 *            <li> useResultAsParam : 이전에 처리된 요청의 결과를 다음 요청에 매개 변수로 넘길지 여부를
 *            설정한다. 요청 결과를 넘기면 true 이며, 기본 값은 false 이다. </li>
 *            <li> stopOnFailure : 이전 요청이 실패하는 경우 다음 요청을 멈출지 여부를 설정한다. 멈춘다면 true
 *            이며, 기본 값은 false 이다. </li>
 *            </ul>
 * @constructor
 * @since 1.3.7
 * 
 * @example // $Ajax 요청 큐를 생성한다. var oAjaxQueue = new $Ajax.Queue({
 ...
 *            name 옵션의 이름
 * @param {Object}
 *            [value] 옵션의 값. 옵션을 설정하는 경우에는 값을 지정한다.
 * @return {Value | $Ajax.Queue} 값을 가져오는 경우 해당 값을, 설정하는 경우 $Ajax.Queue 객체를 리턴한다.
 * @example var oAjaxQueue = new $Ajax.Queue({ useResultAsParam : true });
 * 
 ...
 *            요청 시 전송할 매개 변수 객체
 * 
 * @example var oAjax1 = new $Ajax('ajax_test.php',{ onload : function(res){ //
 *          onload 핸들러 } }); var oAjax2 = new $Ajax('ajax_test.php',{ onload :
 *          function(res){ // onload 핸들러 } }); var oAjax3 = new
 *          $Ajax('ajax_test.php',{ onload : function(res){ // onload 핸들러 }
 * 
 * });
 * 
 * var oAjaxQueue = new $Ajax.Queue({ async : true, useResultAsParam : true,
 * stopOnFailure : false });
 *  // Ajax 요청 큐에 추가한다. oAjaxQueue.add(oAjax1,{seq:1});
 * oAjaxQueue.add(oAjax2,{seq:2,foo:99}); oAjaxQueue.add(oAjax3,{seq:3});
 * 
 * oAjaxQueue.request();
 * 
 */
 ...
 *  // 서버에 Ajax 요청을 보낸다. oAjaxQueue.request();
 * 
 */
 ...
 * @param {Object}
 *            hashObject 해시로 만들 객체.
 * @return {$H} 해시 객체
 * @constructor
 * @example var h = $H({one:"first", two:"second", three:"third"})
 * @author Kim, Taegon
 * 
 */
 ...
 * @example var hash = $H({one:"first", two:"second"});
 *  // 값을 설정할 때 hash.$("three", "third");
 *  // hash => {one:"first", two:"second", three:"third"}
 *  // 값을 반환할 때 var three = hash.$("three");
 *  // three => "third"
 * 
 */
 ...
 *            callback 실행할 콜백 함수
 * @param {Object}
 *            thisObject 콜백 함수의 this
 * @example function printIt(value, key) { document.write(key+" => "+value+"
 *          <br>
 *          "); } $H({one:"first", two:"second",
 *          three:"third"}).forEach(printIt);
 * 
 */
...
/**
 * 
 * filter 메서드는 해시 객체에서 필터 콜백 함수를 만족하는 원소를 수집한다. 수집한 원소는 새로운 $H 객체의 원소가 된다. 콜백함수는
 * Boolean 값을 반환해야 한다.
 * 
 * @param {Function}
 *            callback 필터 콜백 함수
 ...
 * @example function callback(value, key, object) { // value 해시의 값 // key 해시의
 *          고유한 키 혹은 이름 // object JavaScript Core Object 객체 }
 * 
 */
 ...
 *          고유한 키 혹은 이름 // object JavaScript Core Object 객체
 * 
 * var r = key+"_"+value; document.writeln (r + "<br />
 * "); return r; } $H({one:"first", two:"second", three:"third"}).map(callback);
 * 
 */
 ...
 *            key 제거할 원소의 키
 * @return {void} 제거한 키 값
 * @example var h = $H({one:"first", two:"second", three:"third"}); h.remove
 *          ("two"); // h의 해시 테이블은 {one:"first", three:"third"}
 * 
 */
 ...
 * @example var h = $H({one:"first", two:"second", three:"third"}); h.search
 *          ("second"); // two
 * 
 * h.search ("fist"); // false
 * 
 ...
 * @example var h = $H({one:"first", two:"second", three:"third"});
 *          h.hasKey("four"); // false h.hasKey("one"); // true
 * 
 */
 ...
 * @see $H#ksort
 * @example var h = $H({one:"하나", two:"둘", three:"셋"}); h.sort (); // {two:"둘",
 *          three:"셋", one:"하나"}
 * 
 */
 ...
 * @return {$H} 원소를 정렬한 해시 객체
 * @see $H#sort
 * @example var h = $H({one:"하나", two:"둘", three:"셋"}); h.sort (); // h =>
 ...
 * @example var h = $H({one:"first", two:"second", three:"third"}); h.keys (); //
 *          ["one", "two", "three"]
 * @see $H#values
 * 
 */
 ...
 * @example var h = $H({one:"first", two:"second", three:"third"}); h.values(); //
 *          ["first", "second", "third"]
 * @see $H#keys
 * 
 */
 ...
 * @return {String}
 * @example var h = $H({one:"first", two:"second", three:"third"});
 *          h.toQueryString(); // "one=first&two=second&three=third"
 * 
 */
 ...
 * 
 * @return {$H} 비워진 해시 객체
 * @example var hash = $H({a:1, b:2, c:3}); // hash => {a:1, b:2, c:3}
 * 
 * hash.empty(); // hash => {}
 * 
 */
 ...
 * 
 * @remark forEach, filter, map와 같은 루프를 중단한다. 강제로 exception을 발생시키므로 try ~ catch
 *         영역에서 이 메소드를 실행하면 정상적으로 동작하지 않을 수 있다.
 * @example $H({a:1, b:2, c:3}).forEach(function(v,k,o) { ... if (k == "b")
 *          $H.Break(); ... });
 * @see $H.Continue
 * 
 */
 ...
 * 
 * @remark forEach, filter, map와 같은 루프 실행 도중에 현재 루프를 중단하고 다음으로 넘어간다. 강제로
 *         exception을 발생시키므로 try ~ catch 영역에서 이 메소드를 실행하면 정상적으로 동작하지 않을 수 있다.
 * @example $H({a:1, b:2, c:3}).forEach(function(v,k,o) { ... if (v % 2 == 0)
 *          $H.Continue(); ... });
 * @see $H.Break
 * 
 */
 ...
 * $Json 객체를 리턴한다.
 * 
 * @class $Json 객체는 자바스크립트에서 JSON(JavaScript Object Notation)을 다루기 위한 다양한 메서드를
 *        제공한다.
 * @param {Object |
 *            String} sObject 객체, 혹은 JSON으로 인코딩 가능한 문자열.
 * @return {$Json} 인수를 인코딩한 $Json 객체.
 * @remark XML 형태의 문자열을 사용하여 $Json 객체를 생성하려면 $Json#fromXML 메서드를 사용한다.
 * @example var oStr = $Json ('{ zoo: "myFirstZoo", tiger: 3, zebra: 2}');
 * 
 * var d = {name : 'nhn', location: 'Bundang-gu'} var oObj = $Json (d);
 * 
 * @constructor
 * @author Kim, Taegon
 * 
 ...
 * 
 * jindo.$Json._makeJson = function(sObject){ if
 * (window.JSON&&window.JSON.parse) { jindo.$Json._makeJson = function(sObject){
 * if (typeof sObject == "string") { try{ return JSON.parse(sObject); }catch(e){
 * return jindo.$Json._oldMakeJSON(sObject); } } return sObject; } }else{
 * jindo.$Json._makeJson = function(sObject){ if (typeof sObject == "string") {
 * return jindo.$Json._oldMakeJSON(sObject); } return sObject; } } return
 * jindo.$Json._makeJson(sObject); }
 */
...
jindo.$Json.fromXML = function(sXML) {
	...
	/*
	 * 
	 * $0 : 전체 $1 : 태그명 $2 : 속성문자열 $3 : 닫는태그 $4 : CDATA바디값 $5 : 그냥 바디값
	 * 
	 */
 ...
 * @example var j = $Json.fromXML('<data><id>Faqh%$</id><str
 *          attr="123">string value</str></data>'); var r = j.get
 *          ("/data/id");
 *  // 결과 : // [Faqh%$]
 * 
 */
 ...
 * @example var j = $Json({foo:1, bar: 31}); document.write (j.toString());
 *          document.write (j);
 *  // 결과 : // {"bar":31,"foo":1}{"bar":31,"foo":1}
 * 
 */
 ...
 * @example var json = $Json({foo:1, bar: 31}); json.toXML();
 *  // 결과 : // <foo>1</foo><bar>31</bar>
 * 
 */
 ...
 * @example var json = $Json({foo:1, bar: 31}); json.toObject();
 *  // 결과 : // {foo: 1, bar: 31}
 * 
 */
 ...
 * 
 * @return {Boolean} 불린값.
 * @example $Json({foo:1, bar: 31}).compare({foo:1, bar: 31});
 *  // 결과 : // true
 * 
 ...
 * @example var cookie = $Cookie(); cookie.set("session_id1", "value1", 1);
 *          cookie.set("session_id2", "value2", 1); cookie.set("session_id3",
 *          "value3", 1);
 * 
 * document.write (cookie.keys ());
 *  // 결과 : // session_id1, session_id2, session_id3
 * 
 */
 ...
 *            sValue 쿠키 값
 * @param {Number}
 *            [nDays] 쿠키 유효 시간. 유효 시간은 일단위로 설정한다. 유효시간을 생략했다면 쿠키는 웹 브라우저가 종료되면
 *            없어진다.
 * @param {String}
 *            [sDomain] 쿠키 도메인
 * @param {String}
 ...
 *  // 결과 : // value1
 * 
 * cookie.remove("session_id1"); document.write (cookie.get ("session_id1"));
 *  // 결과 : // null
 * 
 */
 ...
 *        래핑이란 자바스크립트의 함수를 한번 더 감싸 본래 함수의 기능은 그대로 유지하면서 확장된 기능을 속성 형태로 제공하는 것을
 *        말한다.
 * @constructor
 * @description [Lite]
 * @author Kim, Taegon
 ...
 *            <br>
 *            매개 변수가 $Element 이면 전달된 매개 변수를 그대로 반환하며 undefined 혹은 null 인 경우에는
 *            null 을 반환한다.
 * @return {$Element} 생성된 $Element 객체
 * 
 * @example var element = $Element($("box")); // HTML 엘리먼트를 매개 변수로 지정 var
 *          element = $Element("box"); // HTML 엘리먼트의 id를 매개 변수로 지정 var element =
 *          $Element("<DIV>"); // 태그를 매개 변수로 지정, DIV 엘리먼트를 생성하여 래핑함
 * 
 */
 ...
 * @description [Lite]
 * 
 * @example var element = $Element("sample_div"); element.$value(); // 원래의 엘리먼트가
 *          반환된다
 * 
 */
 ...
 * 
 * @param {Boolean}
 *            [bVisible] 화면에 보여줄지의 여부<br>
 *            매개 변수를 생략하면 HTML 엘리먼트의 현재 display 속성을 확인하여 true/false로 반환한다.(none
 *            이면 false)<br>
 *            매개 변수가 true인 경우에는 display 속성을 설정하고 false인 경우에는 display 속성을 none 으로
 *            변경한다.
 * @param {String}
 *            [sDisplay] display 속성 값<br>
 ...
 * @since 1.4.5부터 bVisible 매개 변수의 값이 true 인 경우 sDisplay 매개 변수의 값으로 display 속성을
 *        설정할 수 있다.
 * @see $Element#show
 * @see $Element#hide
 * @see $Element#toggle
 * 
 * @example <div id="sample_div" style="display:none">Hello world</div>
 *  // 조회 $Element("sample_div").visible(); // false
 * 
 * @example // 화면에 보이도록 설정 $Element("sample_div").visible(true, 'block');
 * 
 * //Before <div id="sample_div" style="display:none">Hello world</div>
 * 
 * //After <div id="sample_div" style="display:block">Hello world</div>
 * 
 */
 ...
 * 
 * @example // 화면에 보이도록 설정 $Element("sample_div").show();
 * 
 * //Before <div id="sample_div" style="display:none">Hello world</div>
 * 
 * //After <div id="sample_div" style="display:block">Hello world</div>
 ...
 * @see $Element#visible
 * 
 * @example // 화면에 보이지 않도록 설정 $Element("sample_div").hide();
 * 
 * //Before <div id="sample_div" style="display:block">Hello world</div>
 * 
 * //After <div id="sample_div" style="display:none">Hello world</div>
 * 
 */
 ...
 * 
 * @param {String}
 *            [sDisplay] 보이도록 변경할 때 사용되는 display 속성 값
 * @returns {$Element} display 속성을 변경한 $Element 객체
 * @description [Lite]
 * @see $Element#show
 * @see $Element#hide
 ...
 * @example // 화면에 보이거나, 보이지 않도록 처리 $Element("sample_div1").toggle();
 *          $Element("sample_div2").toggle();
 * 
 * //Before <div id="sample_div1" style="display:block">Hello</div> <div
 * id="sample_div2" style="display:none">Good Bye</div>
 * 
 * //After <div id="sample_div1" style="display:none">Hello</div> <div
 * id="sample_div2" style="display:block">Good Bye</div>
 * 
 */
 ...
 * @description [Lite]
 * 
 * @example <div id="sample" style="background-color:#2B81AF; width:20px;
 *          height:20px;"></div>
 *  // 조회 $Element("sample").opacity(); // 1
 * 
 * @example // 투명도 값 설정 $Element("sample").opacity(0.4);
 * 
 * //Before <div style="background-color: rgb(43, 129, 175); width: 20px;
 * height: 20px;" id="sample"></div>
 * 
 * //After <div style="background-color: rgb(43, 129, 175); width: 20px; height:
 * 20px; opacity: 0.4;" id="sample"></div>
 * 
 */
 ...
 * 
 * @remark CSS 속성은 Camel 표기법을 사용한다. 따라서 border-width-bottom은 borderWidthBottom으로
 *         정의한다.
 * @remark float 속성은 Javascript의 예약어로 사용하므로 css 메서드에서는 float 대신 cssFloat를 사용한다.
 *         (Internet Explorer에서는 styleFloat를, 그 외의 브라우저에서는 cssFloat를 사용한다.)
 * @param {String |
 *            Object | $H} sName CSS 속성명 혹은 하나 이상의 CSS 속성과 값을 가지는 객체.<br>
 ...
 * @param {String |
 *            Number} [sValue] CSS 속성에 설정할 값.<br>
 *            단위가 필요한 값은 숫자 혹은 단위를 포함한 문자열을 사용한다.
 * @return {String | $Element} 값을 가져올 때는 값을 문자열로 반환하고, 값을 설정할 때는 값을 설정한 현재의
 *         $Element 를 반환한다.
 * @description [Lite]
 * @example <style type="text/css"> #btn { width: 120px; height: 30px;
 *          background-color: blue; } </style>
 * 
 * <span id="btn"></span>
 * 
 * ...
 *  // CSS 속성 값 조회 $Element('btn').css('backgroundColor'); // rgb (0, 0, 255)
 * 
 * @example // CSS 속성 값 설정 $Element('btn').css('backgroundColor', 'red');
 * 
 * //Before <span id="btn"></span>
 * 
 * //After <span id="btn" style="background-color: red;"></span>
 * 
 * @example // 여러개의 CSS 속성 값을 설정 $Element('btn').css({ width: "200px", // 200
 *          height: "80px" // 80 으로 설정하여도 결과는 같음 });
 * 
 * //Before <span id="btn" style="background-color: red;"></span>
 * 
 * //After <span id="btn" style="background-color: red; width: 200px; height:
 * 80px;"></span>
 * 
 */
	...
	if (e.currentStyle) {
		...
		fpGetCss = function(e, sName) {
			try {
				...
				var sVal = (e.style[sName] || d.defaultView.getComputedStyle(e,
						null).getPropertyValue(
						sName.replace(/([A-Z])/g, "-$1").toLowerCase()));
...
/**
 * 
 * DOM 엘리먼트의 HTML 속성을 가져오거나 설정한다. 하나의 인자만 사용하면 해당 HTML 속성의 속성 값을 가져온다. 해당 속성이
 * 없다면 null을 반환한다. 두 개의 인자를 사용하면 첫 번째 인자에 해당하는 HTML 속성의 속성 값을 두 번째 인자의 값으로 설정한다.
 * 첫 번째 인자로 Object 혹은 $H 객체를 사용하면 두 개 이상의 HTML 속성을 한꺼번에 정의할 수 있다.
 * 
 * @param {String|Object|$H}
 *            sName HTML 속성 이름 혹은 설정값 객체
 * @param {String|Number}
 *            [sValue] 설정 값. 설정 값을 null로 지정하면 해당 HTML 속성을 삭제한다.
 * @return {String|$Element} 값을 가져올 때는 String 설정 값을, 값을 설정할 때는 값을 설정한 현재의
 *         $Element를 리턴한다.
 * @description [Lite]
 * 
 * @example <a href="http://www.naver.com" id="sample_a" target="_blank">Naver</a>
 * 
 * $Element("sample_a").attr("href"); // http://www.naver.com
 * @example $Element("sample_a").attr("href", "http://www.hangame.com/");
 * 
 * //Before <a href="http://www.naver.com" id="sample_a" target="_blank">Naver</a>
 * //After <a href="http://www.hangame.com" id="sample_a" target="_blank">Naver</a>
 * @example $Element("sample_a").attr({ "href" : "http://www.hangame.com",
 *          "target" : "_self" })
 * 
 * //Before <a href="http://www.naver.com" id="sample_a" target="_blank">Naver</a>
 * //After <a href="http://www.hangame.com" id="sample_a" target="_self">Naver</a>
 * 
 */
 ...
 * 
 * @remark width 메서드는 HTML 엘리먼트의 실제 너비를 가져온다. 브라우저마다 Box 모델의 크기 계산 방법이 다르므로 CSS의
 *         width 속성 값과 width 메서드의 반환 값은 서로 다를 수 있다.
 * @param {Number}
 *            [width] 설정할 너비 값. 단위는 px 이며 매개 변수의 값은 숫자로 지정한다.<br>
 *            매개 변수를 생략하면 너비 값을 가져온다.
 * @return {Number | $Element} 값을 가져오는 경우에는 HTML 엘리먼트의 실제 너비를, 값을 설정하는 경우에는 너비
 *         값이 변경된 현재의 $Element 객체를 반환한다.
 * @description [Lite]
 * @see $Element#height
 * 
 * @example <style type="text/css"> div { width:70px; height:50px; padding:5px;
 *          margin:5px; background:red; } </style>
 * 
 * <div id="sample_div"></div>
 * 
 * ...
 *  // 조회 $Element("sample_div").width(); // 80
 * 
 * @example // 위의 예제 HTML 엘리먼트에 너비 값을 설정 $Element("sample_div").width(200);
 * 
 * //Before <div id="sample_div"></div>
 * 
 * //After <div id="sample_div" style="width: 190px"></div>
 * 
 */
 ...
 * 
 * @remark height 메서드는 HTML 엘리먼트의 실제 높이를 가져온다. 브라우저마다 Box 모델의 크기 계산 방법이 다르므로
 *         CSS의 height 속성 값과 height 메서드의 반환 값은 서로 다를 수 있다.
 * @param {Number}
 *            [height] 설정할 높이 값. 단위는 px 이며 매개 변수의 값은 숫자로 지정한다.
 * @return {Number | $Element} 값을 가져오는 경우에는 HTML 엘리먼트의 실제 높이를, 값을 설정하는 경우에는 높이
 *         값이 변경된 현재의 $Element 객체를 반환한다.
 * @description [Lite]
 * @see $Element#width
 * 
 ...
 * 
 * ...
 *  // 조회 $Element("sample_div").height(); // 60
 * 
 * @example // 위의 예제 HTML 엘리먼트에 높이 값을 설정 $Element("sample_div").height(100);
 * 
 * //Before <div id="sample_div"></div>
 * 
 * //After <div id="sample_div" style="height: 90px"></div>
 * 
 */
 ...
 * 
 * @example <style type="text/css"> p { margin: 8px; font-size:16px; } .selected {
 *          color:#0077FF; } .highlight { background:#C6E746; } </style>
 * 
 * <p>
 * Hello and <span id="sample_span" class="selected">Goodbye</span>
 * </p>
 * 
 * ...
 *  // 클래스명 조회 $Element("sample_span").className(); // selected
 * 
 * @example // 위의 예제 HTML 엘리먼트에 클래스명 설정 welSample.className("highlight");
 * 
 * //Before
 * <p>
 * Hello and <span id="sample_span" class="selected">Goodbye</span>
 ...
 * //After
 * <p>
 * Hello and <span id="sample_span" class="highlight">Goodbye</span>
 * </p>
 * 
 */
 ...
 * 
 * <p>
 * Hello and <span id="sample_span" class="selected highlight">Goodbye</span>
 * </p>
 * 
 * ...
 *  // 클래스의 사용여부를 확인 var welSample = $Element("sample_span");
 * welSample.hasClass("selected"); // true welSample.hasClass("highlight"); //
 * true
 * 
 */
 ...
 * 
 * @param {String}
 *            sClass 추가할 클래스명. 여러개의 클래스명을 추가하려면 공백으로 구분한다.
 ...
 * @see $Element#toggleClass
 * 
 * @example // 클래스 추가 $Element("sample_span1").addClass("selected");
 *          $Element("sample_span2").addClass("selected highlight");
 * 
 * //Before
 * <p>
 * Hello and <span id="sample_span1">Goodbye</span>
 * </p>
 * <p>
 * Hello and <span id="sample_span2">Goodbye</span>
 * </p>
 * 
 * //After
 * <p>
 * Hello and <span id="sample_span1" class="selected">Goodbye</span>
 * </p>
 * <p>
 * Hello and <span id="sample_span2" class="selected highlight">Goodbye</span>
 * </p>
 * 
 */
 ...
 * 
 * @param {String}
 *            sClass 제거할 클래스명. 여러개의 클래스명을 제거하려면 공백으로 구분한다.
 ...
 * @see $Element#toggleClass
 * 
 * @example // 클래스 제거 $Element("sample_span").removeClass("selected");
 * 
 * //Before
 * <p>
 * Hello and <span id="sample_span" class="selected highlight">Goodbye</span>
 ...
 * Hello and <span id="sample_span" class="highlight">Goodbye</span>
 * </p>
 * @example // 여러개의 클래스를 제거 $Element("sample_span").removeClass("selected
 *          highlight"); $Element("sample_span").removeClass("highlight
 *          selected");
 * 
 * //Before
 * <p>
 ...
 * //After
 * <p>
 * Hello and <span id="sample_span" class="">Goodbye</span>
 * </p>
 * 
 */
 ...
 * 
 * @param {String}
 *            sClass 추가 혹은 제거할 클래스명
 * @param {String}
 *            [sClass2] 추가 혹은 제거할 클래스명<br>
 ...
 * @see $Element#removeClass
 * 
 * @example // 매개 변수가 하나인 경우 $Element("sample_span1").toggleClass("highlight");
 *          $Element("sample_span2").toggleClass("highlight");
 * 
 * //Before
 * <p>
 * Hello and <span id="sample_span1" class="selected highlight">Goodbye</span>
 * </p>
 * <p>
 * Hello and <span id="sample_span2" class="selected">Goodbye</span>
 * </p>
 * 
 * //After
 * <p>
 ...
 * </p>
 * 
 * @example // 매개 변수가 두 개인 경우 $Element("sample_span1").toggleClass("selected",
 *          "highlight"); $Element("sample_span2").toggleClass("selected",
 *          "highlight");
 * 
 * //Before
 * <p>
 * Hello and <span id="sample_span1" class="highlight">Goodbye</span>
 * </p>
 * <p>
 * Hello and <span id="sample_span2" class="selected">Goodbye</span>
 * </p>
 ...
 * </p>
 * <p>
 * Hello and <span id="sample_span2" class="highlight">Goodbye</span>
 * </p>
 * 
 */
 ...
 * 
 * ...
 *  // 텍스트 노드 값 조회 $Element("sample_ul").text(); // 결과 // 하나 // 둘 // 셋 // 넷
 * @example // 텍스트 노드 값 설정 $Element("sample_ul").text('다섯');
 * 
 * //Before
 * <ul id="sample_ul">
 * <li>하나</li>
 ...
 * 다섯
 * </ul>
 * @example // 텍스트 노드 값 설정 $Element("sample_p").text("New Content");
 * 
 * //Before
 * <p id="sample_p">
 * Old Content
...
jindo.$Element.prototype.text = function(sText) {
	...
	if (type != "undefined"
		...
		try {
			/*
			 * 
			 * Opera 11.01에서 textContext가 Get일때 정상적으로 동작하지 않음. 그래서 get일 때는
			 * innerText을 사용하고 set하는 경우는 textContent을
			 * 사용한다.(http://devcafe.nhncorp.com/ajaxui/295768)
			 * 
			 */
			if (prop != "value")
 ...
 * @description [Lite]
 * 
 * @example <div id="sample_container">
 *          <p>
 *          <em>Old</em> content
 *          </p>
 *          </div>
 * 
 * ...
 *  // 내부 HTML 조회 $Element("sample_container").html(); //
 * <p>
 * <em>Old</em> content
 * </p>
 * 
 * @example // 내부 HTML 설정 $Element("sample_container").html("
 *          <p>
 *          New <em>content</em>
 *          </p>
 *          ");
 * 
 * //Before <div id="sample_container">
 * <p>
 * <em>Old</em> content
 * </p>
 * </div>
 * 
 * //After <div id="sample_container">
 * <p>
 * New <em>content</em>
 * </p>
 * </div>
	...
	if (isIe) {
		jindo.$Element.prototype.html = function(sHTML) {
			if (typeof sHTML != "undefined" && arguments.length) {
				...
				/*
				 * 
				 * IE 나 FireFox 의 일부 상황에서 SELECT 태그나 TABLE, TR, THEAD, TBODY 태그에
				 * innerHTML 을 셋팅해도 문제가 생기지 않도록 보완 - hooriza
				 * 
				 */
				...
				if (oDummy) {
					...
					if (oFound) {
						...
						for (var oChild; oChild = oEl.firstChild;)
							oChild.removeNode(true); // innerHTML = '';

						for (var oChild = oFound.firstChild; oChild; oChild = oFound.firstChild) {
							if (sTag == 'select') {
								/*
								 * 
								 * ie에서 select테그일 경우 option중 selected가 되어 있는
								 * option이 있는 경우 중간에 selected가 되어 있으면 그 다음 부터는
								 * 계속 selected가 true로 되어 있어 해결하기 위해 cloneNode를
								 * 이용하여 option을 카피한 후 selected를 변경함. - mixed
								 * 
								 */
		...
		jindo.$Element.prototype.html = function(sHTML) {
			if (typeof sHTML != "undefined" && arguments.length) {
				...
				if (!oEl.parentNode) {
					/*
					 * 
					 * IE 나 FireFox 의 일부 상황에서 SELECT 태그나 TABLE, TR, THEAD, TBODY
					 * 태그에 innerHTML 을 셋팅해도 문제가 생기지 않도록 보완 - hooriza
					 * 
					 */
					var sId = 'R' + new Date().getTime()
							+ parseInt(Math.random() * 100000, 10);
 ...
 * <div id="sample1">
 * <p>
 * <span id="sample2">Sample</span> content
 * </p>
 * </div>
 * 
 * ...
 *  // 외부 HTML 값을 조회 $Element("sample0").outerHTML(); //
 * <h2 id="sample0">Today is...</h2>
 * 
 * $Element("sample1").outerHTML(); // <div id="sample1">
 * <p>
 * <span id="sample2">Sample</span> content
 * </p>
 * </div>
 ...
 * 
 * @param {Number}
 *            duration HTML 엘리먼트가 완전히 나타날 때까지 걸리는 시간. 단위는 초를 사용한다.
 * @param {Function}
 *            [callback] HTML 엘리먼트가 완전히 나타난 후에 실행할 콜백 함수.
 * @return {$Element} 현재의 $Element 객체
 * 
 * @remark IE6 에서 filter 사용 시 해당 엘리먼트가 position 속성을 가지고 있으면 사라지는 문제가 있기 때문에 HTML
 *         엘리먼트에 position 속성이 없어야 한다.
 * @remark webkit기반의 브라우저(safari5+, mobile safari, chrome, mobile webkit),
 *         opera10.60+ 에서는 CSS3 transition을 사용한다. 그 이외의 브라우저에서는 자바스크립트를 사용한다.
 * 
 * @see $Element#show
 * @see $Element#disappear
 * 
 * @example $Element("sample1").appear(5, function(){
 *          $Element("sample2").appear(3); });
 * 
 * //Before <div style="display: none; background-color: rgb(51, 51, 153);
 * width: 100px; height: 50px;" id="sample1"> <div style="display: none;
 * background-color: rgb(165, 10, 81); width: 50px; height: 20px;" id="sample2">
 * </div> </div>
 * 
 * //After(1) : sample1 엘리먼트가 나타남 <div style="display: block; background-color:
 * rgb(51, 51, 153); width: 100px; height: 50px; opacity: 1;" id="sample1"> <div
 * style="display: none; background-color: rgb(165, 10, 81); width: 50px;
 * height: 20px;" id="sample2"> </div> </div>
 * 
 * //After(2) : sample2 엘리먼트가 나타남 <div style="display: block; background-color:
 * rgb(51, 51, 153); width: 100px; height: 50px; opacity: 1;" id="sample1"> <div
 * style="display: block; background-color: rgb(165, 10, 81); width: 50px;
 * height: 20px; opacity: 1;" id="sample2"> </div> </div>
 * 
 */
 ...
 * 
 * @param {Number}
 *            duration HTML 엘리먼트 완전히 사라질 때까지 걸리는 시간. 단위는 초를 사용한다.
 * @param {Function}
 *            [callback] HTML 엘리먼트가 완전히 사라진 후에 실행할 콜백 함수.
 * @return {$Element} 현재의 $Element 객체
 * 
 * @remark webkit기반의 브라우저(safari5+, mobile safari, chrome, mobile webkit),
 *         opera10.60+ 에서는 CSS3 transition을 사용한다. 그 이외의 브라우저에서는 자바스크립트를 사용한다.
 ...
 * @see $Element#appear
 * 
 * @example $Element("sample1").disappear(5, function(){
 *          $Element("sample2").disappear(3); });
 * 
 * //Before <div id="sample1" style="background-color: rgb(51, 51, 153); width:
 * 100px; height: 50px;"> </div> <div id="sample2" style="background-color:
 * rgb(165, 10, 81); width: 100px; height: 50px;"> </div>
 * 
 * //After(1) : sample1 엘리먼트가 사라짐 <div id="sample1" style="background-color:
 * rgb(51, 51, 153); width: 100px; height: 50px; opacity: 1; display: none;">
 * </div> <div id="sample2" style="background-color: rgb(165, 10, 81); width:
 * 100px; height: 50px;"> </div>
 * 
 * //After(2) : sample2 엘리먼트가 사라짐 <div id="sample1" style="background-color:
 * rgb(51, 51, 153); width: 100px; height: 50px; opacity: 1; display: none;">
 * </div> <div id="sample2" style="background-color: rgb(165, 10, 81); width:
 * 100px; height: 50px; opacity: 1; display: none;"> </div>
 * 
 */
 ...
 * 
 * @param {Number}
 *            [nTop] 문서의 맨 위에서 HTML 엘리먼트 맨 위까지의 거리. 단위는 px
 * @param {Number}
 *            [nLeft] 문서의 왼쪽 가장자리에서 HTML 엘리먼트 왼쪽 가장자리까지의 거리. 단위는 px
 ...
 * @remark 일부 브라우저와 일부 상황에서 inline 엘리먼트에 대한 위치가 올바르게 얻어지지 않는 문제가 있으며 이 경우 해당
 *         엘리먼트를 position:relative; 로 바꿔주는 식으로 해결할 수 있다.
 * @author Hooriza
 * 
 * @example <style type="text/css"> div { background-color:#2B81AF; width:20px;
 *          height:20px; float:left; left:100px; top:50px; position:absolute;}
 *          </style>
 * 
 * <div id="sample"></div>
 * 
 * ...
 *  // 위치 값 조회 $Element("sample").offset(); // { left=100, top=50 }
 * 
 * @example // 위치 값 설정 $Element("sample").offset(40, 30);
 * 
 * //Before <div id="sample"></div>
 * 
 * //After <div id="sample" style="top: 40px; left: 30px;"></div>
 * 
 */
 ...
 * &lt;script&gt; 태그가 포함된 문자열을 매개 변수로 지정하면, &lt;script&gt; 안에 있는 내용을 파싱하여 eval 를
 * 수행한다.
 * 
 * @param {String}
 *            sHTML &lt;script&gt; 태그가 포함된 HTML 문자열
 * @return {$Element} 현재의 $Element 객체를 반환
 * 
 * @example // script 태그가 포함된 문자열을 지정 var response = "<script
 *          type='text/javascript'>$Element('sample').appendHTML('
 *          <li>4</li>')</script>";
 * 
 ...
 * 
 * @param {$Element |
 *            HTML Element | String} oElement 추가할 HTML 엘리먼트. 문자열, HTML 엘리먼트, 혹은
 *            $Element 을 매개 변수로 지정할 수 있다.<br>
 *            <br>
 ...
 * 
 * @example // id 가 sample1 인 HTML 엘리먼트에 // id 가 sample2 인 HTML 엘리먼트를 추가
 *          $Element("sample1").append("sample2");
 * 
 * //Before <div id="sample2"> <div>Hello 2</div> </div> <div id="sample1">
 * <div>Hello 1</div> </div>
 * 
 * //After <div id="sample1"> <div>Hello 1</div> <div id="sample2"> <div>Hello
 * 2</div> </div> </div>
 * 
 * @example // id 가 sample 인 HTML 엘리먼트에 // 새로운 DIV 엘리먼트를 추가 var elChild = $("<div>Hello
 *          New</div>"); $Element("sample").append(elChild);
 * 
 * //Before <div id="sample"> <div>Hello</div> </div>
 * 
 * //After <div id="sample"> <div>Hello </div> <div>Hello New</div> </div>
 * 
 */
 ...
 * 
 * @param {$Element |
 *            HTMLElement | String} oElement 추가할 HTML 엘리먼트. 문자열, HTML 엘리먼트, 혹은
 *            $Element 을 매개 변수로 지정할 수 있다.<br>
 *            <br>
 ...
 * @see $Element#prependTo
 * 
 * @example // id 가 sample1 인 HTML 엘리먼트에서 // id 가 sample2 인 HTML 엘리먼트를 첫 번째
 *          자식노드로 이동 $Element("sample1").prepend("sample2");
 * 
 * //Before <div id="sample1"> <div>Hello 1</div> <div id="sample2"> <div>Hello
 * 2</div> </div> </div>
 * 
 * //After <div id="sample1"> <div id="sample2"> <div>Hello 2</div> </div>
 * <div>Hello 1</div> </div>
 * 
 * @example // id 가 sample 인 HTML 엘리먼트에 // 새로운 DIV 엘리먼트를 추가 var elChild = $("<div>Hello
 *          New</div>"); $Element("sample").prepend(elChild);
 * 
 * //Before <div id="sample"> <div>Hello</div> </div>
 * 
 * //After <div id="sample"> <div>Hello New</div> <div>Hello</div> </div>
 * 
 */
 ...
 * 
 * @param {$Element |
 *            HTML Element | String} oElement 대체할 HTML 엘리먼트. 문자열, HTML 엘리먼트, 혹은
 *            $Element 을 매개 변수로 지정할 수 있다.<br>
 *            <br>
 ...
 * 
 * @example // id 가 sample1 인 HTML 엘리먼트에서 // id 가 sample2 인 HTML 엘리먼트로 대체
 *          $Element('sample1').replace('sample2');
 * 
 * //Before <div> <div id="sample1">Sample1</div> </div> <div
 * id="sample2">Sample2</div>
 * 
 * //After <div> <div id="sample2">Sample2</div> </div>
 * 
 * @example // 새로운 DIV 엘리먼트로 대체 $Element("btn").replace($("<div>Sample</div>"));
 * 
 * //Before <button id="btn">Sample</button>
 * 
 * //After <div>Sample</div>
 * 
 */
 ...
 * 
 * @param {$Element |
 *            HTML Element | String} oElement 부모가 될 HTML 엘리먼트. <br>
 *            <br>
 ...
 * @see $Element#prependTo
 * 
 * @example // id 가 sample2 인 HTML 엘리먼트에 // id 가 sample1 인 HTML 엘리먼트를 추가
 *          $Element("sample1").appendTo("sample2");
 * 
 * //Before <div id="sample1"> <div>Hello 1</div> </div> <div id="sample2">
 * <div>Hello 2</div> </div>
 * 
 * //After <div id="sample2"> <div>Hello 2</div> <div id="sample1"> <div>Hello
 * 1</div> </div> </div>
 * 
 */
 ...
 * 
 * @param {$Element |
 *            HTML Element | String} oElement 부모가 될 HTML 엘리먼트. 문자열, HTML 엘리먼트,
 *            혹은 $Element 을 매개 변수로 지정할 수 있다.<br>
 *            <br>
 *            매개 변수가 문자열이면 해당 문자열을 id 로 하는 HTML 엘리먼트를 부모로 한다.<br>
 *            매개 변수가 HTML 엘리먼트이면 해당 엘리먼트를 부모로 한다.<br>
 *            매개 변수가 $Element 이면 $Element 객체 내부의 HTML 엘리먼트를 부모로 한다.
 ...
 * <div>Hello 2</div> </div>
 * 
 * //After <div id="sample2"> <div id="sample1"> <div>Hello 1</div> </div>
 * <div>Hello 2</div> </div>
 * 
 */
 ...
 * 
 * @param {$Element |
 *            HTML Element | String} oElement 추가할 HTML 엘리먼트.<br>
 *            <br>
 *            문자열, HTML 엘리먼트, 혹은 $Element 을 매개 변수로 지정할 수 있다.<br>
 *            <br>
 ...
 * @see $Element#prependTo
 * 
 * @example // id 가 sample1 인 HTML 엘리먼트 앞에 // id 가 sample2 인 HTML 엘리먼트를 추가 함
 *          $Element("sample1").before("sample2"); // sample2를 래핑한 $Element 를 반환
 * 
 * //Before <div id="sample1"> <div>Hello 1</div> <div id="sample2"> <div>Hello
 * 2</div> </div> </div>
 * 
 * //After <div id="sample2"> <div>Hello 2</div> </div> <div id="sample1">
 * <div>Hello 1</div> </div>
 * 
 * @example // 새로운 DIV 엘리먼트를 추가 var elNew = $("<div>Hello New</div>");
 *          $Element("sample").before(elNew); // elNew 엘리먼트를 래핑한 $Element 를 반환
 * 
 * //Before <div id="sample"> <div>Hello</div> </div>
 * 
 * //After <div>Hello New</div> <div id="sample"> <div>Hello</div> </div>
 * 
 */
 ...
 * @see $Element#prependTo
 * 
 * @example // id 가 sample1 인 HTML 엘리먼트 뒤에 // id 가 sample2 인 HTML 엘리먼트를 추가 함
 *          $Element("sample1").after("sample2"); // sample2를 래핑한 $Element 를 반환
 * 
 * //Before <div id="sample1"> <div>Hello 1</div> <div id="sample2"> <div>Hello
 * 2</div> </div> </div>
 * 
 * //After <div id="sample1"> <div>Hello 1</div> </div> <div id="sample2">
 * <div>Hello 2</div> </div>
 * 
 * @example // 새로운 DIV 엘리먼트를 추가 var elNew = $("<div>Hello New</div>");
 ...
 * //Before <div id="sample"> <div>Hello</div> </div>
 * 
 * //After <div id="sample"> <div>Hello</div> </div> <div>Hello New</div>
 * 
 */
 ...
 * 
 * @param {Function}
 *            [pFunc] 상위 엘리먼트 노드의 검색 조건을 지정한 콜백 함수.<br>
 ...
 * @param {Number}
 *            [limit] 탐색할 상위 엘리먼트 노드의 뎁스.<br>
 *            매개 변수를 생략하면 모든 상위 엘리먼트 노드를 탐색한다.<br>
 *            pFunc 매개 변수를 null로 설정하고 limit 매개 변수를 설정하면 제한된 뎁스의 상위 엘리먼트 노드를 조건
 *            없이 검색한다.
 ...
 * @see $Element#indexOf
 * 
 * @example <div class="sample" id="div1"> <div id="div2"> <div class="sample"
 *          id="div3"> <div id="target"> Sample <div id="div4"> Sample </div>
 *          <div class="sample" id="div5"> Sample </div> </div> <div
 *          class="sample" id="div6"> Sample </div> </div> </div> </div>
 * 
 * <script type="text/javascript"> var welTarget = $Element("target"); var
 * parent = welTarget.parent(); // id가 div3인 DIV를 래핑한 $Element를 반환
 * 
 * parent = welTarget.parent(function(v){ return v.hasClass("sample"); }); //
 * id가 div3인 DIV를 래핑한 $Element와 // id가 div1인 DIV를 래핑한 $Element를 원소로 하는 배열을 반환
 * 
 * parent = welTarget.parent(function(v){ return v.hasClass("sample"); }, 1); //
 * id가 div3인 DIV를 래핑한 $Element를 원소로 하는 배열을 반환 </script>
 * 
 */
 ...
 * 
 * @param {Function}
 *            [pFunc] 하위 엘리먼트 노드의 검색 조건을 지정한 콜백 함수.<br>
 ...
 * @param {Number}
 *            [limit] 탐색할 하위 엘리먼트 노드의 뎁스.<br>
 *            매개 변수를 생략하면 모든 하위 엘리먼트 노드를 탐색한다.<br>
 *            pFunc 매개 변수를 null로 설정하고 limit 매개 변수를 설정하면 제한된 뎁스의 하위 엘리먼트 노드를 조건
 *            없이 검색한다.
 ...
 * @see $Element#indexOf
 * 
 * @example <div class="sample" id="target"> <div id="div1"> <div class="sample"
 *          id="div2"> <div id="div3"> Sample <div id="div4"> Sample </div> <div
 *          class="sample" id="div5"> Sample <div class="sample" id="div6">
 *          Sample </div> </div> </div> </div> </div> <div class="sample"
 *          id="div7"> Sample </div> </div>
 * 
 * <script type="text/javascript"> var welTarget = $Element("target"); var child =
 * welTarget.child(); // id가 div1인 DIV를 래핑한 $Element와 // id가 div7인 DIV를 래핑한
 * $Element를 원소로 하는 배열을 반환
 * 
 * child = welTarget.child(function(v){ return v.hasClass("sample"); }); // id가
 * div2인 DIV를 래핑한 $Element와 // id가 div5인 DIV를 래핑한 $Element와 // id가 div6인 DIV를
 * 래핑한 $Element와 // id가 div7인 DIV를 래핑한 $Element를 원소로 하는 배열을 반환
 * 
 * child = welTarget.child(function(v){ return v.hasClass("sample"); }, 1); //
 * id가 div7인 DIV를 래핑한 $Element를 원소로 하는 배열을 반환
 * 
 * child = welTarget.child(function(v){ return v.hasClass("sample"); }, 2); //
 * id가 div2인 DIV를 래핑한 $Element와 // id가 div7인 DIV를 래핑한 $Element를 원소로 하는 배열을 반환
 * </script>
 * 
 */
 ...
 * 
 * @param {Function}
 *            [pFunc] 이전 형제 엘리먼트 노드의 검색 조건을 지정한 콜백 함수.<br>
 ...
 * @return {$Element | Array} 바로 전의 형제 엘리먼트 노드를 가리키는 $Element 혹은 조건에 맞는 형제 노드의
 *         $Element의 배열을 반환한다.
 * 
 * @see $Element#parent
 * @see $Element#child
 ...
 * @see $Element#indexOf
 * 
 * @example <div class="sample" id="sample_div1"> <div id="sample_div2"> <div
 *          class="sample" id="sample_div3"> Sample1 </div> <div
 *          id="sample_div4"> Sample2 </div> <div class="sample"
 *          id="sample_div5"> Sample3 </div> <div id="sample_div"> Sample4 <div
 *          id="sample_div6"> Sample5 </div> </div> <div id="sample_div7">
 *          Sample6 </div> <div class="sample" id="sample_div8"> Sample7 </div>
 *          </div> </div>
 * 
 * <script type="text/javascript"> var sibling = $Element("sample_div").prev(); //
 * id가 sample_div5인 DIV를 래핑한 $Element를 반환
 * 
 * sibling = $Element("sample_div").prev(function(v){ return
 * $Element(v).hasClass("sample"); }); // id가 sample_div5인 DIV를 래핑한 $Element와 //
 * id가 sample_div3인 DIV를 래핑한 $Element를 원소로 하는 배열을 반환 </script>
 * 
 */
 ...
 * 
 * @param {Function}
 *            [pFunc] 다음 형제 엘리먼트 노드의 검색 조건을 지정한 콜백 함수.<br>
 *            매개 변수를 생략하면 바로 다음의 형제 엘리먼트 노드를 반환하고,<br>
 *            매개 변수로 콜백 함수를 지정하면 콜백 함수의 실행 결과가 true 인 형제 엘리먼트 노드의 배열을 반환한다.<br>
 *            콜백 함수에 매개 변수로 탐색 중인 형제 엘리먼트 노드를 넘긴다. ($Element 객체가 아님)
 * @return {$Element | Array} 바로 다음의 형제 엘리먼트 노드를 가리키는 $Element 혹은 조건에 맞는 형제 노드의
 *         $Element의 배열을 반환한다.
 * 
 * @see $Element#parent
 * @see $Element#child
 ...
 *          </div> </div>
 * 
 * <script type="text/javascript"> var sibling = $Element("sample_div").next(); //
 * id가 sample_div7인 DIV를 래핑한 $Element를 반환
 * 
 * sibling = $Element("sample_div").next(function(v){ return
 * $Element(v).hasClass("sample"); }); // id가 sample_div8인 DIV를 래핑한 $Element를
 * 원소로 하는 배열을 반환 </script>
 * 
 */
 ...
 * @see $Element#indexOf
 * 
 * @example <div id="sample_div1"> <div id="sample_div2"> <div id="sample_div">
 *          Sample1 <div id="sample_div3"> <div id="sample_div4"> Sample2 </div>
 *          Sample3 </div> <div id="sample_div5"> Sample4 <div id="sample_div6">
 *          Sample5 </div> </div> </div> </div> </div>
 * 
 * <script type="text/javascript"> var firstChild =
 * $Element("sample_div").first(); // id가 sample_div3인 DIV를 래핑한 $Element를 반환
 * </script>
 * 
 */
 ...
 *          Sample5 </div> </div> </div> </div> </div>
 * 
 * <script type="text/javascript"> var lastChild =
 * $Element("sample_div").last(); // id가 sample_div5인 DIV를 래핑한 $Element를 반환
 * </script>
 * 
 */
 ...
 * 
 * @param {HTML
 *            Element | String | $Element} element 부모 노드인지 확인할 HTML 엘리먼트.<br>
 *            <br>
 *            문자열, HTML 엘리먼트, 혹은 $Element 을 매개 변수로 지정할 수 있다.<br>
 *            <br>
 ...
 * 
 * @example <div id="parent"> <div id="child"> <div id="grandchild"></div>
 *          </div> </div> <div id="others"></div>
 * 
 * ...
 *  // 부모/자식 확인하기 $Element("child").isChildOf("parent"); // 결과 : true
 ...
 * 
 * @param {HTML
 *            Element | String | $Element} element 자식 노드인지 확인할 HTML 엘리먼트. 문자열,
 *            HTML 엘리먼트, 혹은 $Element 을 매개 변수로 지정할 수 있다가매개 변수가 문자열이면 해당 문자열을 id 로
 *            하는 HTML 엘리먼트를 확인한다.<br>
 *            매개 변수가 HTML 엘리먼트이면 해당 엘리먼트를 확인한다.<br>
 *            매개 변수가 $Element 이면 $Element 객체 내부의 HTML 엘리먼트를 확인한다.
 ...
 * 
 * @example <div id="parent"> <div id="child"></div> </div> <div id="others"></div>
 * 
 * ...
 *  // 부모/자식 확인하기 $Element("parent").isParentOf("child"); // 결과 : true
...
/**
 * 
 * isChildOf , isParentOf의 기본이 되는 API(IE에서는 contains,기타 브라우져에는
 * compareDocumentPosition을 사용하고 둘다 없는 경우는 기존 레거시 API사용.)
 * 
 * @param {HTMLElement}
 *            eParent 부모노드
 * @param {HTMLElement}
 ...
 * 
 * @param {HTML
 *            Element | String | $Element} element 같은 HTML 엘리먼트인지 확인할 HTML 엘리먼트.
 *            문자열, HTML 엘리먼트, 혹은 $Element 을 매개 변수로 지정할 수 있다.<br>
 *            <br>
 *            매개 변수가 문자열이면 해당 문자열을 id 로 하는 HTML 엘리먼트를 확인한다.<br>
 *            매개 변수가 HTML 엘리먼트이면 해당 엘리먼트를 확인한다.<br>
 ...
 * 
 * @example <div id="sample1"><span>Sample</span></div> <div id="sample2"><span>Sample</span></div>
 * 
 * ...
 *  // 같은 HTML 엘리먼트인지 확인 var welSpan1 = $Element("sample1").first(); //
 * <span>Sample</span> var welSpan2 = $Element("sample2").first(); //
 * <span>Sample</span>
 * 
 * welSpan1.isEqual(welSpan2); // 결과 : false welSpan1.isEqual(welSpan1); // 결과 :
 * true
 * 
 */
 ...
 * 
 * @param {String}
 *            sEvent 실행할 이벤트 이름. on 접두사는 생략한다.
 * @param {Object}
 *            [oProps] 이벤트 실행 시 사용할 이벤트 객체의 속성을 지정한다.
 * @return {$Element} 이벤트가 발생한 HTML 엘리먼트
 * 
 * @since WebKit 계열에서는 이벤트 객체의 keyCode 가 read-only 인 관계로 key 이벤트를 발생시킬 경우
 *        keyCode 값이 설정되지 않는다. 1.4.1 부터 keyCode 값을 설정할 수 있다.
 * 
 * @example $Element("div").fireEvent("click", {left : true, middle : false,
 *          right : false}); // click 이벤트 발생
 *          $Element("div").fireEvent("mouseover", {screenX : 50, screenY : 50,
 *          clientX : 50, clientY : 50}); // mouseover 이벤트 발생
 *          $Element("div").fireEvent("keydown", {keyCode : 13, alt : true,
 *          shift : false ,meta : false, ctrl : true}); // keydown 이벤트 발생
 * 
 */
 ...
 * @see $Element#remove
 * 
 * @example // 자식 노드를 모두 제거 $Element("sample").empty();
 * 
 * //Before <div id="sample"><span>노드</span> <span>모두</span> 삭제하기 </div>
 * 
 * //After <div id="sample"></div>
 * 
 */
 ...
 * 
 * @param {HTML
 *            Element | String | $Element} oChild 제거할 자식 엘리먼트 노드.<br>
 *            <br>
 *            문자열, HTML 엘리먼트, 혹은 $Element 을 매개 변수로 지정할 수 있다.<br>
 *            <br>
 ...
 * @see $Element#leave
 * 
 * @example // 특정 자식 노드를 제거 $Element("sample").remove("child2");
 * 
 * //Before <div id="sample"><span id="child1">노드</span> <span
 * id="child2">삭제하기</span></div>
 * 
 * //After <div id="sample"><span id="child1">노드</span> </div>
 * 
 */
 ...
 * @see $Element#remove
 * 
 * @example // 부모 엘리먼트 노드에서 제거 $Element("sample").leave();
 * 
 * //Before <div> <div id="sample"><span>노드</span> <span>모두</span> 삭제하기
 * </div> </div>
 * 
 * //After // <div id="sample"><span>노드</span> <span>모두</span> 삭제하기 </div>를
 * 래핑한 $Element가 반환된다 <div>
 * 
 * </div>
 * 
 */
 ...
 * 
 * @param {String |
 *            HTML Element | $Element} wrapper 감쌀 HTML 엘리먼트. 문자열, HTML 엘리먼트, 혹은
 *            $Element 을 매개 변수로 지정할 수 있다.<br>
 *            <br>
 ...
 * 
 * @example $Element("sample1").wrap("sample2");
 * 
 * //Before <div id="sample1"><span>Sample</span></div> <div id="sample2"><span>Sample</span></div>
 * 
 * //After <div id="sample2"><span>Sample</span><div id="sample1"><span>Sample</span></div></div>
 * 
 * @example $Element("box").wrap($('<DIV>'));
 * 
 * //Before <span id="box"></span>
 * 
 * //After <div><span id="box"></span></div>
 * 
 */
 ...
 * @remark 브라우저에서의 HTML 엘리먼트의 너비를 기준으로 텍스트 노드의 길이를 정하므로 HTML 엘리먼트는 반드시 보이는 상태여야
 *         한다.
 * @remark 화면에 전체 텍스트 노드가 보였다가 줄어드는 경우가 있다. 이런 경우, HTML 엘리먼트에서 overflow:hidden
 *         속성을 활용한다.
 * 
 * @param {String}
 *            [stringTail] 말줄임 표시자. <br>
 *            매개 변수에 지정한 문자열을 텍스트 노드 맨 끝에 붙이고 텍스트 노드의 길이를 조절한다.<br>
 *            매개 변수를 생락하면 말줄임표('...')를 사용한다.
 * 
 * @example $Element("sample_span").ellipsis();
 * 
 * //Before <div style="width:300px; border:1px solid #CCCCCC; padding:10px">
 * <span id="sample_span">NHN은 검색과 게임을 양축으로 혁신적이고 편리한 온라인 서비스를 꾸준히 선보이며 디지털 라이프를
 * 선도하고 있습니다.</span> </div>
 * 
 * //After <div style="width:300px; border:1px solid #CCCCCC; padding:10px">
 * <span id="sample_span">NHN은 검색과 게임을 양축으로 혁신적...</span> </div>
 * 
 */
 ...
 * 
 * @param {String |
 *            HTML Element | $Element} element 확인할 HTML 엘리먼트. 문자열, HTML 엘리먼트, 혹은
 *            $Element 을 매개 변수로 지정할 수 있다.<br>
 *            <br>
 *            매개 변수가 문자열이면 해당 문자열을 id 로 하는 HTML 엘리먼트를 사용한다.<br>
 *            매개 변수가 HTML 엘리먼트이면 해당 엘리먼트를 사용한다.<br>
 ...
 * @see $Element#last
 * 
 * @example <div id="sample_div1"> <div id="sample_div"> <div id="sample_div2">
 *          Sample1 </div> <div id="sample_div3"> <div id="sample_div4"> Sample2
 *          </div> Sample3 </div> <div id="sample_div5"> Sample4 <div
 *          id="sample_div6"> Sample5 </div> </div> </div> </div>
 * 
 * <script type="text/javascript"> var welSample = $Element("sample_div");
 ...
 * welSample.indexOf($Element("sample_div6")); // 결과 : -1 </script>
 * 
 */
 ...
 * 
 * @example <div id="sample"> <div></div> <div class="pink"></div> <div></div>
 *          <div class="pink"></div> <div></div> <div class="blue"></div>
 *          <div class="blue"></div> </div>
 * 
 * <script type="text/javascript"> $Element("sample").queryAll(".pink"); // <div
 * class="pink"></div>와 <div class="pink"></div>를 원소로 하는 배열을 반환
 * 
 * $Element("sample").queryAll(".green"); // [] 빈 배열을 반환 </script>
 * 
 */
 ...
 *          <div class="blue"></div> </div>
 * 
 * <script type="text/javascript"> $Element("sample").query(".pink"); // 첫 번째
 * <div class="pink"></div> DIV 엘리먼트를 반환
 * 
 * $Element("sample").query(".green"); // null 을 반환 </script>
 * 
 */
 ...
 * @see $Element#queryAll
 * 
 * @example <div id="sample" class="blue"></div>
 * 
 * <script type="text/javascript"> $Element("sample").test(".blue"); // 결과 :
 * true $Element("sample").test(".red"); // 결과 : false </script>
 * 
 */
 ...
 * 
 * @example <div id="sample"> <div> <div>1</div> <div>2</div> <div>3</div>
 *          <div>4</div> <div>5</div> <div>6</div> </div> </div>
 * 
 * <script type="text/javascript"> $Element("sample").xpathAll("div/div[5]"); //
 * <div>5</div> 엘리먼트를 원소로 하는 배열이 반환 됨 </script>
 * 
 */
 ...
 * 
 * @example // 내부 HTML 가장 뒤에 덧붙이기 $Element("sample_ul").appendHTML("
 *          <li>3</li>
 *          <li>4</li>
 *          ");
 * 
 ...
 * @see $Element#afterHTML
 * 
 * @example // 내부 HTML 가장 앞에 삽입 $Element("sample_ul").prependHTML("
 *          <li>3</li>
 *          <li>4</li>
 *          ");
 * 
 ...
 * @see $Element#afterHTML
 * 
 * @example var welSample = $Element("sample_ul");
 * 
 ...
 * 
 * <ul>
 * <li><a href="http://devcode.nhncorp.com/projects/jindo/wiki/EventDelegate"
 * target="_blank">Event Delegate 란?</a></li>
 * <ul>
 * 
 * @param {String}
 *            sEvent 이벤트 이름. on 접두사는 생략한다.<br>
 *            domready, mousewheel, mouseenter, mouseleave 은 지원하지 않는다.<br>
 * 
 * @param {String |
 *            Function} vFilter 원하는 HTML 엘리먼트에 대해서만 이벤트를 실행하도록 하기 위한 필터이다.<br>
 *            <br>
 ...
 *            필터 함수의 첫 번째 매개 변수는 HTML 엘리먼트 자신이고, 두 번째 매개 변수는 이벤트가 발생한 HTML
 *            엘리먼트이다.
 * 
 * @param {Function}
 *            fpCallback 필터에서 true 가 반환된 경우 실행되는 콜백 함수이다.<br>
 *            매개 변수로 이벤트 객체가 지정된다.
 * 
 ...
 * <li>4</li>
 * </ul>
 *  // CSS 셀렉터를 필터로 사용하는 경우 $Element("parent").delegate("click", ".odd", // 필터
 * function(eEvent){ // 콜백 함수 alert("odd 클래스를 가진 li가 클릭 될 때 실행"); });
 * @example
 * <ul id="parent">
 * <li class="odd">1</li>
 * <li>2</li>
 ...
 * <li>4</li>
 * </ul>
 *  // 함수를 필터로 사용하는 경우 $Element("parent").delegate("click",
 * function(oEle,oClickEle){ // 필터 return oClickEle.innerHTML == "2" },
 * function(eEvent){ // 콜백 함수 alert("클릭한 엘리먼트의 innerHTML이 2인 경우에 실행"); });
 * 
 */
 ...
 * 
 * @param {String}
 *            sEvent 이벤드 델리게이션 등록 시 사용한 이벤트 이름. on 접두사는 생략한다.
 * @param {String|Function}
 *            vFilter 이벤트 델리게이션 등록 시 사용한 필터.
 * @param {Function}
 *            fpCallback 이벤트 델리게이션 등록 시 사용한 콜백 함수.
 * @return {$Element} $Element 객체를 리턴한다.
 * @since 1.4.6부터 사용 가능.
 ...
 * <li>4</li>
 * </ul>
 *  // 콜백 함수 var fnOddClass = function(eEvent){ alert("odd 클래스를 가진 li가 클릭 될 때
 * 실행"); };
 * 
 ...
 *            sEvent 이벤트 타입.
 * @param {String|Function}
 *            vFilter cssquery,function 이렇게 2가지 타입이 들어옴.
 * @param {Function}
 *            fpCallback 해당 cChecker에 들어오는 함수가 맞을때 실행되는 함수.
 ...
 * 
 * @param {Object}
 *            aDataOfEvent 이벤트와 함수를 가지고 있는 오브젝트.
 * @param {String|Function}
 *            vFilter cssquery,check하는 함수.
 * @param {Function}
 *            fpCallback 실행될 함수.
 * @param {Function}
 *            fpCheck 체크하는 함수.
...
jindo.$Element.prototype._unbind = function(sEvent, vFilter, fpCallback) {
	...
	if (sEvent && vFilter && fpCallback) {
		...
		for (var i = 0, l = aEvents.length; i < l; i++) {
			...
			jindo.$Element._unEventBind(oEle, sEachEvent, oEle["_delegate_"
					+ sEachEvent + "_func"]);
 ...
 * @param {Object |
 *            String} thisObject <br>
 *            함수가 특정 객체의 메서드일 때, 해당 객체도 같이 전달한다. 혹은 함수의 몸체를 나타내는 문자열.
 * @return {$Fn} $Fn 객체
 * @see $Fn#toFunction
 * @description [Lite]
 * @example func : function() { // code here }
 * 
 * var fn = $Fn(func, this);
 * @example var someObject = { func : function() { // code here } }
 * 
 * var fn = $Fn(someObject.func, someObject);
 * 
 * @example var fn = $Fn("a, b", "return a + b;"); var result = fn.$value()(1,
 *          2) // result = 3;
 ...
 * @example func : function() { // code here }
 * 
 * var fn = $Fn(func, this); fn.$value(); // 원래의 함수가 리턴된다.
 * 
 */
 ...
 * @description [Lite]
 * @example var sName = "OUT"; var oThis = { sName : "IN" };
 * 
 * function getName() { return this.sName; }
 * 
 * oThis.getName = $Fn(getName, oThis).bind();
 * 
 * alert( getName() ); // OUT alert( oThis.getName() ); // IN
 * 
 * @example // 함수를 미리 선언하고 나중에 사용할 때, // 함수에서 참조 하는 값들은 해당 함수를 생성 할때의 값이 아니라 함수
 *          실행 시점의 값이 사용 되므로 이때 bind를 이용한다. for(var i=0; i<2;i++){ aTmp[i] =
 *          function(){alert(i);} }
 * 
 * for(var n=0; n<2;n++){ aTmp[n](); // 숫자 2만 두번 alert된다. }
 * 
 * for(var i=0; i<2;i++){ aTmp[i] = $Fn(function(nTest){alert(nTest);},
 * this).bind(i); }
 * 
 * for(var n=0; n<2;n++){ aTmp[n](); // 숫자 0, 1이 alert된다. }
 * 
 * @example //클래스 생성 시 함수를 매개변수로 할 때, scope를 맞춰주기 위해 bind를 사용한다. var MyClass =
 *          $Class({ fFunc : null, $init : function(func){ this.fFunc = func;
 * 
 * this.testFunc(); }, testFunc : function(){ this.fFunc(); } }) var MainClass =
 * $Class({ $init : function(){ var oMyClass1 = new MyClass(this.func1); var
 * oMyClass2 = new MyClass($Fn(this.func2, this).bind()); }, func1 : function(){
 * alert(this);// this는 MyClass 를 의미한다. }, func2 : function(){ alert(this);//
 * this는 MainClass 를 의미한다. } }) function init(){ var a = new MainClass(); }
 * 
 */
 ...
 * <li>기본 이벤트 외에 추가로 사용이 가능한 이벤트에는 domready, mouseenter, mouseleave,
 * mousewheel이 있다.</li>
 * </ul>
 * 
 * @param {Element |
 *            Array} oElement 이벤트 핸들러를 할당할 엘리먼트(엘리먼트가 원소인 배열도 가능)
 * @param {String}
 *            sEvent 이벤트 종류
 * @param {Boolean}
 *            bUseCapture capturing을 사용할 때(1.4.2 부터 지원)
 * @see $Fn#detach
 * @description [Lite]
 * @return {$Fn} 생성된 $Fn 객체
 * @example var someObject = { func : function() { // code here } }
 * 
 * $Fn(someObject.func, someObject).attach($("test"),"click"); // 단일 엘리먼트에 클릭을
 * 할당한 경우 $Fn(someObject.func, someObject).attach($$(".test"),"click"); // 여러
 * 엘리먼트에 클릭을 할당한 경우 //attach에 첫번째 인자로 엘리먼트 배열이 들어오면 해당 모든 엘리먼트에 이벤트가 바인딩 됨.
 * 
 */
jindo.$Fn.prototype.attach = function(oElement, sEvent, bUseCapture) {
	...
	if (typeof el.addEventListener != "undefined") {
		...
		} else if (ev == "animationstart" || ev == "animationend"
 ...
 * @example var someObject = { func : function() { // code here } }
 * 
 * $Fn(someObject.func, someObject).detach($("test"),"click"); // 단일 엘리먼트에 클릭을
 * 할당한 경우 $Fn(someObject.func, someObject).detach($$(".test"),"click"); // 여러
 * 엘리먼트에 클릭을 할당한 경우
 * 
 */
 ...
 * 
 * @param {Number}
 *            nSec 함수를 호출할 때까지 대기할 시간(초 단위).
 * @param {Array}
 *            args 함수를 호출할 때 사용할 매개변수. 매개변수가 여러 개일 경우 배열을 사용한다.
 * @see $Fn#bind
 * @see $Fn#setInterval
 * @description [Lite]
 * @return {$Fn} 생성된 $Fn 객체
 * @example function func(a, b) { alert(a + b); }
 * 
 ...
 * @example function func(a, b) { alert(a + b); }
 * 
 * var fpDelay = $Fn(func); fpDelay.delay(5, [3, 5]); fpDelay.stopDelay();
 * 
 */
 ...
 * @example function func(a, b) { alert(a + b); }
 * 
 * var fpDelay = $Fn(func); fpDelay.repeat(5, [3, 5]); fpDelay.stopRepeat();
 * 
 */
...
jindo.$Fn.prototype.free = function(oElement) {
	...
	while (len > 0) {
		...
		/*
		 * 
		 * unload시에 엘리먼트에 attach한 함수를 detach하는 로직이 있는데 해당 로직으로 인하여 unload이벤트가
		 * 실행되지 않아 실행시키는 로직을 만듬. 그리고 해당 로직은 gc에서 호출할때만 호출.
		 * 
		 */
 ...
 * 
 * @example var someObject = { func1 : function() { // code here }, func2 :
 *          function() { // code here } }
 * 
 ...
 * 
 * @class $Event 클래스는 자바스크립트 Event 객체의 래퍼(Wrapper) 클래스이다. 사용자는 $Event.element
 *        메서드를 사용하여 이벤트가 발생한 객체를 알 수 있다.
 * @param {Event}
 *            e Event 객체
 * @constructor
 * @description [Lite]
 ...
 * 
 * @description [Lite]
 * @example function eventHandler(evt) { var mouse = evt.mouse();
 * 
 * mouse.delta; // Number. 휠이 움직인 정도. 휠을 위로 굴리면 양수, 아래로 굴리면 음수. mouse.left; //
 * Boolean. 마우스 왼쪽 버튼을 눌렸으면 true, 아니면 false mouse.middle; // Boolean. 마우스 중간 버튼을
 * 눌렸으면 true, 아니면 false mouse.right; // Boolean. 마우스 오른쪽 버튼을 눌렸으면 true, 아니면
 * false }
 ...
 * 
 * @description [Lite]
 * @example function eventHandler(evt) { var key = evt.key();
 * 
 * key.keyCode; // Number. 눌린 키보드의 키코드 key.alt; // Boolean. Alt 키를 눌렸으면 true.
 * key.ctrl; // Boolean. Ctrl 키를 눌렸으면 true. key.meta; // Boolean. Meta 키를 눌렸으면
 * true. Meta키는 맥의 커맨드키를 검출할 때 사용합니다. key.shift; // Boolean. Shift 키를 눌렸으면 true.
 * key.up; // Boolean. 위쪽 화살표 키를 눌렸으면 true. key.down; // Boolean. 아래쪽 화살표 키를
 * 눌렸으면 true. key.left; // Boolean. 왼쪽 화살표 키를 눌렸으면 true. key.right; // Boolean.
 * 오른쪽 화살표 키를 눌렸으면 true. key.enter; // Boolean. 리턴키를 눌렀으면 true key.esc; //
 * Boolean. ESC키를 눌렀으면 true } }
 ...
 * 
 * @param {Boolean}
 *            bGetOffset 현재 엘리먼트에 대한 마우스 커서의 상대위치인 offsetX, offsetY를 구할 것인지의 여부.
 *            true면 값을 구한다(offsetX, offsetY는 1.2.0버전부터 추가). $Element 가 포함되어 있어야
 *            한다.
 * @description [Lite]
 * @example function eventHandler(evt) { var pos = evt.pos();
 * 
 * pos.clientX; // Number. 현재 화면에 대한 X 좌표 pos.clientY; // Number. 현재 화면에 대한 Y 좌표
 * pos.pageX; // Number. 문서 전체에 대한 X 좌표 pos.pageY; // Number. 문서 전체에 대한 Y 좌표
 ...
 * 
 * @remark 버블링은 특정 HTML 엘리먼트에서 이벤트가 발생했을 때 이벤트가 상위 노드로 전파되는 현상이다. 예를 들어, div 객체를
 *         클릭할 때 div와 함께 상위 엘리먼트인 document에도 onclick 이벤트가 발생한다. stop() 메소드는 지정한
 *         객체에서만 이벤트가 발생하도록 버블링을 차단한다.
 * @description [Lite]
 * @example // 기본 동작만 중지시키고 싶을 때 (1.1.3버전 이상) function stopDefaultOnly(evt) { //
 *          Here is some code to execute
 *  // Stop default event only evt.stop($Event.CANCEL_DEFAULT); }
 * @return {$Event} 이벤트 객체.
 * @param {Number}
 *            nCancel 이벤트의 버블링과 기본 동작을 선택하여 중지시킨다. 기본값은 $Event.CANCEL_ALL
 *            이다(1.1.3 버전 이상).
 * 
 */
 ...
 * 
 * @example function eventHandler(evt){ evt.$value(); }
 * @return {Event} Event
 * 
 */
 ...
 * @param {String |
 *            Array} els 문서에서 DOM 엘리먼트를 찾기 위한 CSS 선택자 혹은 id, HTMLElement,
 *            $Element의 배열
 * @constructor
 ...
 * 
 * @param {Number}
 *            idx 가져올 엘리먼트의 인덱스. 인덱스는 0에서 부터 시작한다.
 ...
 * @param {Number}
 *            [nLen] 새로 리턴할 배열의 크기. nLen이 기존의 배열보다 크면 oValue으로 초기화한 원소를 마지막에
 *            덧붙인다. nLen이 기존 배열보다 작으면 nLen번째 이후의 원소는 제거한다.
 * @param {Value}
 *            [oValue] 새로운 원소를 추가할 때 사용할 초기값
 * @see $A#length
 * 
 ...
 * @see $S#toString
 * @example var str = $S("Hello world!!"); str.$value();
 *  // 결과 : // Hello world!!
 * 
 */
 ...
 * @return {String} 래핑된 원래의 문자열
 * @remark $value와 같은 의미
 * @example var str = $S("Hello world!!"); str.toString();
 *  // 결과 : // Hello world!!
 * 
 */
 ...
 * @example var str = " I have many spaces. "; document.write ( $S(str).trim() );
 *  // 결과 : // I have many spaces.
 * 
 */
 ...
 * @see $S#unescapeHTML
 * @remark ", &, <, > ,' 를 각각 &quot;, &amp;, &lt;, &gt; &#39;로 변경한다.
 * @example var str = ">_<;;"; document.write( $S(str).escapeHTML() );
 *  // 결과 : // &amp;gt;_&amp;lt;;;
 * 
 */
 ...
 * @example var str = "Meeting <b>people</b> is easy."; document.write(
 *          $S(str).stripTags() );
 *  // 결과 : // Meeting people is easy.
 * 
 */
 ...
 * @example document.write ( $S("Abc").times(3) );
 *  // 결과 : AbcAbcAbc
 * 
 */
 ...
 * @see $S#escapeHTML
 * @example var str = "&lt;a
 *          href=&quot;http://naver.com&quot;&gt;Naver&lt;/a&gt;";
 *          document.write( $S(str).unescapeHTML() );
 *  // 결과 : // <a href="http://naver.com">Naver</a>
 * 
 */
 ...
 * @see $S#escapeHTML
 * @example var str = '가"\'나\\'; document.write( $S(str).escape() );
 *  // 결과 : \uAC00\"\'\uB098\\
 * 
 */
...
/**
 * 
 * bytes 메서드는 문자열의 실제 바이트(byte) 수를 리턴하고, 제한하려는 바이트(byte) 수를 지정하면 문자열을 해당 크기에 맞게
 * 잘라낸다.(1.4.3 부터 charset사용 가능)
 * 
 * @return 문자열의 바이트 수. 단, 첫번째 매개변수를 설정하면 자기 객체($S)를 리턴한다.
 * @param {Number|Object}
 *            nBytes 맞출 문자열의 바이트(byte) 수 | charset을 지정할 때 사용
 * @remark 문서의 charset을 해석해서 인코딩 방식에 따라 한글을 비롯한 유니코드 문자열의 바이트 수를 계산한다.
 * @example // 문서가 euc-kr 환경임을 가정합니다.
 * 
 * var str = "한글과 English가 섞인 문장..."; document.write( $S(str).bytes() );
 *  // 결과 : // 37
 * 
 ...
 * @example var str = "aa=first&bb=second"; var obj = $S(str).parseString();
 *  // 결과 : // obj => { aa : "first", bb : "second" }
 * 
 */
 ...
 * @example var str = "Slash / is very important. Backslash \ is more important.
 *          +_+"; document.write( $S(str).escapeRegex() );
 *  // 결과 : \/ is very important\. Backslash \\ is more important\.
 * \+_\+
 * 
 */
...
/**
 * 
 * format 메서드는 문자열을 형식 문자열에 대입하여 새로운 문자열을 만든다. 형식 문자열은 %로 시작하며, 형식 문자열의 종류는 <a
 * href="http://www.php.net/manual/en/function.sprintf.php">PHP</a>와 동일하다.
 * 
 * @param {String}
 *            formatString 형식 문자열
 * @return {String} 문자열을 형식 문자열에 대입하여 만든 새로운 문자열.
 * @example var str = $S("%4d년 %02d월 %02d일").format(2008, 2, 13);
 *  // 결과 : // str = "2008년 02월 13일"
 * 
 ...
 * @param {Document}
 *            doc 기능에 사용된 document 객체. 기본값은 현재 문서의 document.
 * @constructor
 * @author Hooriza
 * 
 */
 ...
 * @example var size = $Document().scrollSize(); alert('가로 : ' + size.width + ' /
 *          세로 : ' + size.height);
 * 
 */
 ...
 * @example var size = $Document().scrollPosition(); alert('가로 : ' + size.left + ' /
 *          세로 : ' + size.top);
 * @since 1.3.5
 * 
 */
 ...
 * 
 * @return {Object} 가로크기는 width, 세로크기는 height 라는 키값으로 리턴된다
 * @example var size = $Document(document).clientSize(); alert('가로 : ' +
 *          size.width + ' / 세로 : ' + size.height);
 * 
 */
jindo.$Document.prototype.clientSize = function() {
	...
	/*
	 * 
	 * 사파리의 경우 윈도우 리사이즈시에 clientWidth,clientHeight값이 정상적으로 나오지 않아서
	 * window.innerWidth,innerHeight로 대체
	 * 
	 */
	return (isSafari) ? {
 ...
 *         <dd>비표준 렌더링 모드</dd>
 *         </dl>
 * @example var mode = $Document().renderingMode(); alert('렌더링 방식 : ' + mode);
 * 
 */
 ...
 * @param {String}
 *            sXPath 엘리먼트의 위치를 지정한 XPath 값
 * @return {Array} path에 해당하는 요소의 배열
 * @example var oDocument = $Document(); alert
 *          (oDocument.xpathAll("body/div/div").length);
 * 
 */
 ...
 * @param {Element |
 *            String} el 폼(form) 엘리먼트, 혹은 폼 엘리먼트의 id. 만약 동일한 id를 두 개 이상의 엘리먼트에서
 *            사용하면 먼저 나오는 엘리먼트를 리턴한다.
 * @constructor
 * @author Hooriza
 * 
 */
 ...
 * 
 * @param {Mixed}
 *            Mixed 인수를 지정하지 않거나 인수를 하나 이상 설정할 수 있다.
 *            <ol>
 ...
 *            <li>매배 변수로 두 개 이상의 문자열을 설정하면, 문자열과 일치하는 name 속성을 가지는 엘리먼트를 모두
 *            탐색하고 값을 쿼리 스트링 형태로 리턴한다. </li>
 *            </ol>
 * @return {String} 쿼리 문자열 형태로 변환한 엘리먼트와 그 값.
 * @example
 * 
 * <form id="TEST"> <input name="ONE" value="1" type="text" /> <input name="TWO"
 * value="2" checked="checked" type="checkbox" /> <input name="THREE"
 * value="3_1" type="radio" /> <input name="THREE" value="3_2" checked="checked"
 * type="radio" /> <input name="THREE" value="3_3" type="radio" /> <select
 * name="FOUR"> <option value="4_1">..</option> <option value="4_2">..</option>
 * <option value="4_3" selected="selected">..</option> </select> </form>
 * <script type="text/javascript"> var form = $Form('TEST');
 * 
 * var allstr = form.serialize(); alert(allstr ==
 * 'ONE=1&TWO=2&THREE=3_2&FOUR=4_3'); // true
 * 
 * var str = form.serialize('ONE', 'THREE'); alert(str == 'ONE=1&THREE=3_2'); //
 * true </script>
 * 
 */
 ...
 * 
 * @param {String}
 *            sKey 얻고자 하는 입력요소 엘리먼트의 name 문자열, 생략시에는 모든 입력요소들을 배열로 리턴한다.
 ...
 * 
 * @param {Mixed}
 *            mixed enable 메서드는 매개 변수의 개수나 종류에 따라 다르게 동작한다. 자세한 사용법은 다음과 같다.
 *            <ol>
 *            <li> 매개 변수로 문자열을 사용하면 문자열과 일치하는 name 속성을 가진 엘리먼트를 탐색한다. 엘리먼트를
 *            발견했다면 엘리먼트의 활성화 여부를 리턴한다.</li>
 *            <li> 매개 변수로 문자열과 불린(Boolean)을 사용하면 문자열과 일치하는 name 속성을 가진 엘리먼트를 탐색한
 *            후, 활성화 여부를 설정한다. </li>
 *            <li> 매개 변수로 객체를 사용할 수 있다. 객체는 속성 값과 name이 일치하는 엘리먼트를 탐색해서 값에 따라
 *            활성화 여부를 설정한다. </li>
 *            </ol>
 * @return {Boolean|$Form} 엘리먼트의 활성화 여부를 가져오거나 엘리먼트의 활성화 여부를 설정한 $Form 객체.
 * @example
 * 
 * <form id="TEST"> <input name="ONE" disabled="disabled" type="text" /> <input
 * name="TWO" type="checkbox" /> </form> <script type="text/javascript"> var
 * form = $Form('TEST');
 * 
 * var one_enabled = form.enable('ONE'); alert(one_enabled === false); // true
 * 
 * form.enable('TWO', false);
 * 
 * form.enable({ 'ONE' : true, 'TWO' : false }); </script>
 * 
 */
 ...
 * 
 * @param {Mixed}
 *            Mixed 정확안 인수 정보는 다음과 같다.
 *            <ol>
 *            <li>매개 변수로 문자열을 설정하면 name 속성이 일치하는 앨리먼트를 탐색하고 값을 리턴한다</li>
 *            <li>매개 변수로 두 개의 문자열과 불린 값을 name 속성이 일치하는 앨리먼트를 탐색하고 값을 설정한다.
 *            </br> checkbox, radio, selectbox는 엘리먼트를 선택/선택 해제 한다.</li>
 *            <li>두 개 이상의 엘리먼트 값을 동시에 지정하고 싶으면 '엘리먼트 이름 : 엘리먼트 값'을 원소로 가지는 객체를
 *            매개 변수로 설정한다.</li>
 *            </ol>
 * @return {String|$Form} 인수로 엘리먼트만 지정했다면 지정한 엘리먼트의 값을, 인수로 폼 엘리먼와 엘리먼트의 값을
 *         지정했다면 $Form 객체를 리턴한다.
 * @example
 * 
 * <form id="TEST"> <input name="ONE" value="1" type="text" /> <input name="TWO"
 * value="2" type="checkbox" /> </form> <script type="text/javascript"> var form =
 * $Form('TEST');
 * 
 * var one_value = form.value('ONE'); alert(one_value === '1'); // true
 * 
 * var two_value = form.value('TWO'); alert(two_value === undefined); // true
 * 
 * form.value('TWO', 2); alert(two_value === '2'); // true
 * 
 * form.value({ 'ONE' : '1111', 'TWO' : '2' }); // form.value('ONE') -> 1111 //
 * form.value('ONE') -> 2 </script>
 * 
 */
 ...
 * 
 * @param {String}
 *            sTargetName 제출할 폼이 있는 윈도우의 이름. sTargetName을 생략하면 기본 타겟
 * @param {String}
 *            fValidation 제출할 폼의 밸리데이션 함수. form 요소를 인자로 받는다.
 * @return {$Form} 데이터를 제출한 $Form 객체.
 * @example var form = $Form(el); form.submit(); form.submit('foo');
 * 
 */
 ...
 * @example var form = $Form(el); form.reset();
 * 
 */
 ...
 *            HTML 엘리먼트 value 값의 문자열을 템 플릿으로 사용하며, value 값이 없는 경우 HTML 엘리먼트의
 *            innerHTML을 템플릿으로 사용한다.<br>
 *            <br>
 ...
 * 
 * @example // 인자가 일반 문자열인 경우 var tpl = $Template("{=service} : {=url}");
 * 
 * @example <textarea id="tpl1"> {=service} : {=url} &lt;/textarea&gt;
 *  // 같은 TEXTAREA 엘리먼트를 템플릿으로 사용한다 var template1 = $Template("tpl1"); //
 * 인자가 HTML 엘리먼트의 id 아이디 var template2 = $Template($("tpl1")); // 인자가 TEXTAREA
 * 엘리먼트인 경우 </script>
 * 
 * @example <script type="text/template" id="tpl2"> {=service} : {=url}
 *          </script>
 *  // 같은 SCRIPT 엘리먼트를 템플릿으로 사용한다 var template1 = $Template("tpl2"); //
 * 인자가 HTML 엘리먼트의 id 아이디 var template2 = $Template($("tpl2")); // 인자가 SCRIPT
 * 엘리먼트인 경우
 * 
 */
jindo.$Template = function(str) {
	...
	if (typeof str == "undefined") {
		str = "";
	} else if ((obj = document.getElementById(str) || str)
			&& obj.tagName
 ...
 * 
 * @param {Object}
 *            data 템플릿에 들어갈 데이터를 가지는 객체<br>
 *            템플릿에 데이터를 적용할 부분은 객체의 프로퍼티(property) 이름으로 찾는다.
 * @return {String} 템플릿을 해석하고 데이터를 적용한 새로운 문자열
 * 
 * @example // 단순 문자열 치환 // {=value} 부분의 값을 치환한다. ﻿var tpl = $Template("Value1 :
 *          {=val1}, Value2 : {=val2}") var data = { val1: "first value", val2:
 *          "second value" };
 * 
 * document.write( tpl.process(data) );
 *  // 결과 // ﻿Value1 : first value, Value2 : second value
 * 
 * @example // ﻿if/elseif/else : 조건문 // 템플릿을 해석할 때 조건문을 판단하여 처리한다. ﻿var tpl=
 *          $Template("{if num >= 7}7보다 크거나 같다.{elseif num <= 5}5보다 작거나
 *          같다.{else}아마 6?{/if}"); var data = { num: 5 };
 * 
 * document.write( tpl.process(data) );
 *  // 결과 // 5보다 작거나 같다.
 * 
 * @example // set : 임시 변수 사용 // set val=1 로 설정하는 경우 {=val} 부분에 1을 대입한다. ﻿var
 *          tpl = $Template("{set val3=val1}Value1 : {=val1}, Value2 : {=val2},
 *          Value3 : {=val3}") var data = { val1: "first value", val2: "second
 *          value" };
 * 
 * document.write( tpl.process(data) );
 *  // 결과 ﻿// Value1 : first value, Value2 : second value, Value3 : first value
 * 
 * @example // ﻿js : JavaScript 사용 // 템플릿을 해석할 때 해당 JavaScript를 실행한다. ﻿var tpl =
 *          $Template("Value1 : {js $S(=val1).bytes()}, Value2 : {=val2}") var
 *          data = { val1: "first value", val2: "second value" };
 * 
 * document.write( tpl.process(data) );
 *  // 결과 // ﻿Value1 : 11, Value2 : second value
 * 
 * @example // ﻿for in : 반복문(인덱스를 사용하지 않는 경우) ﻿var tpl = $Template("
 *          <h1>포탈 사이트</h1>
 *          \n
 *          <ul>
 *          {for site in portals}\n
 ...
 *          {/for}\n
 *          </ul>
 *          "); var data = { portals: [ { name: "네이버", url :
 *          "http://www.naver.com" }, { name: "다음", url : "http://www.daum.net" }, {
 ...
 * </ul>
 * 
 * @example // ﻿for: 반복문(인덱스를 사용하는 경우) ﻿var tpl = $Template("{for num:word in
 *          numbers}{=word}({=num}){/for}"); var data = { numbers: ["zero",
 *          "one", "two", "three"] };
 * 
 * document.write( tpl.process(data) );
 *  // 결과 // ﻿zero(0) one(1) two(2) three(3)
 * 
 */
...
/**
 * 
 * $Date 객체를 생성하고 리턴한다. ISO문자를 넣은 경우 jindo.$Date.utc을 기반하여 계산된다.
 * 
 * @extends core
 * @class $Date 클래스는 날짜를 처리하기 위한 Date 타입의 레퍼(Wrapper) 클래스이다.
 * @constructor
 * @author Kim, Taegon
 * @example $Date(); $Date(milliseconds); $Date(dateString); //1.4.6 이후 부터 달까지만
 *          넣어도 $Date사용 가능하여 빈 값은 1로 셋팅. $Date(year, month, [date, [hours,
 *          [minitues, [seconds, [milliseconds]]]]]); $Date(2010,6);//이러고 하면
 *          $Date(2010,6,1,1,1,1,1); 와 같음.
 * 
 */
...
/**
 * 
 * names 속성은 $Date에서 사용할 달, 요일, 오전/오후의 이름을 문자열로 저장한다. s_ 를 접두어로 가지는 이름들은
 * 약어(abbreviation)이다.
 * 
 */
jindo.$Date.names = {
...
/**
 * 
 * UTC는 <a href="http://ko.wikipedia.org/wiki/UTC">협정 세계시간</a>으로 한국을 기준으로 기본 값이
 * +9시간이다.
 * 
 * @example jindo.$Date.utc = -10; 을 하면 하와이를 기준으로 계산된다.
 * 
 */
jindo.$Date.utc = 9;
 ...
 * 
 * @param {String}
 *            strDate 날짜, 혹은 시간 형식을 지정한 파싱 대상 문자열
...
/**
 * 
 * format 메서드는 $Date 객체가 저장하고 있는 시간을 인수로 지정한 형식 문자열에 맞추어 변환한다. 형식 문자열은 PHP의 date
 * 함수와 동일하게 사용한다. <table> <caption>날짜</caption> <thead>
 * <tr>
 * <th scope="col">문자</th>
 * <th scope="col">설명</th>
 * <th scope="col">기타</th>
 ...
 * <td>0 ~ 365</td>
 * </tr>
 * </tbody> </table> <table> <caption>월</caption> <thead>
 * <tr>
 * <th scope="col">문자</th>
 * <th scope="col">설명</th>
 * <th scope="col">기타</th>
 ...
 * <td>1 ~ 12</td>
 * </tr>
 * </tbody> </table> <table> <caption>년</caption> <thead>
 * <tr>
 * <th scope="col">문자</th>
 * <th scope="col">설명</th>
 * <th scope="col">기타</th>
 ...
 * <td>10</td>
 * </tr>
 * </tbody> </table> <table> <caption>시</caption> <thead>
 * <tr>
 * <th scope="col">문자</th>
 * <th scope="col">설명</th>
 * <th scope="col">기타</th>
 ...
 * <td>654321</td>
 * </tr>
 * </tbody> </table> <table> <caption>기타</caption> <thead>
 * <tr>
 * <th scope="col">문자</th>
 * <th scope="col">설명</th>
 * <th scope="col">기타</th>
 ...
 * @example var oDate = $Date("Jun 17 2009 12:02:54"); oDate.format("Y.m.d(D) A
 *          H:i") => "2009.06.17(Wed) PM 12:02"
 * 
 */
 ...
 * @param {Number}
 *            nTime 밀리 초 단위의 시간 값.
 * @return {$Date | Number} 인수를 지정했다면 GMT 1970/01/01 00:00:00 에서 부터 인수만큼 지난 시간을
 *         설정한 $DAte 객체. 인수를 지정하지 않았다면 GMT 1970/01/01 00:00:00에서 부터 $Date 객체에
 *         지정된 시각까지 경과한 시간(밀리 초).
 * 
 */
 ...
 * @param {Number}
 *            nYear 설정할 년도값
 * @return {$Date | Number} 인수를 지정하였다면 새로 년도 값을 설정한 $Date 객체. 인수를 지정하지 않았다면
 *         $Date 객체가 지정하고 있는 시각의 년도를 리턴한다.
 * 
 */
 ...
 * @param {Number}
 *            nMon 설정할 달의 값
 * @return {$Date | Number} 인수를 지정하였다면 새로 달을 설정한 $Date 객체. 인수를 지정하지 않았다면 $Date
 *         객체가 지정하고 있는 시각의 달을 리턴한다.
 ...
 * @param {nDate}
 *            nDate 설정할 날짜 값
 * @return {$Date | Number} 인수를 지정하였다면 새로 날짜를 설정한 $Date 객체. 인수를 지정하지 않았다면 $Date
 *         객체가 지정하고 있는 시각의 날짜를 리턴한다.
 * 
 */
 ...
 * @param {Number}
 *            nHour 설정할 시 값
 * @return {$Date | Number} 인수를 지정하였다면 새로 시 값을 설정한 $Date 객체. 인수를 지정하지 않았다면 $Date
 *         객체가 지정하고 있는 시각의 시 값.
 * 
 */
 ...
 * @param {Number}
 *            nMin 설정할 분 값
 * @return {Number} 인수를 지정하였다면 새로 분 값을 설정한 $Date 객체. 인수를 지정하지 않았다면 $Date 객체가
 *         지정하고 있는 시각의 분 값.
 * 
 */
 ...
 * @param {Number}
 *            nSec 설정할 초 값
 * @return {Number} 인수를 지정하였다면 새로 초 값을 설정한 $Date 객체. 인수를 지정하지 않았다면 $Date 객체가
 *         지정하고 있는 시각의 초 값.
 * 
 */
 ...
 * @example $Window().$value(); // 원래의 window 객체를 리턴한다.
 * 
 */
 ...
 * 브라우저에 따라 보안 문제 때문에, 창 크기가 화면의 가시 영역을 벗어나서 커지지 못하도록 막는 경우도 있다. 이 경우에는 지정한 크기보다
 * 작게 창이 커진다.<br>
 * 
 * @param {Number}
 *            nWidth 창의 너비
 ...
 * @see $Window#resizeBy
 * @example // 현재 창의 너비를 400, 높이를 300으로 변경한다. $Window.resizeTo(400, 300);
 * 
 */
 ...
 *            nHeight 늘어날 창의 높이
 * @see $Window#resizeTo
 * @example // 현재 창의 너비를 100, 높이를 50 만큼 늘린다. $Window().resize(100, 50);
 * 
 */
 ...
 * 
 * @param {Number}
 *            nLeft 이동할 x좌표 (pixel 단위)
 * @param {Number}
 *            nTop 이동할 y좌표 (pixel 단위)
 * @see $Window#moveBy
 * @example // 현재 창을 (15, 10) 으로 이동시킨다. $Window().moveTo(15, 10);
 * 
 */
 ...
 * 
 * @param {Number}
 *            nLeft x좌표로 이동할 만큼의 양 (pixel 단위)
 * @param {Number}
 *            nTop y좌표로 이동할 만큼의 양 (pixel 단위)
 * @see $Window#moveTo
 * @example // 현재 창을 좌측으로 15px만큼, 아래로 10px만큼 이동시킨다. $Window().moveBy(15, 10);
 * 
 */
 ...
 * <li>html의 DOCTYPE이 Quirks일때 맥에서는 opera(10), 윈도우에서는 IE6+, opera(10),
 * safari(4)에서 정상적으로 동작하지 않는다.</li>
 * <li>가능 하면 부모창에서 실행 시켜야 한다. 자식창이 모니터 화면을 벗어나 가려져 있을 경우, IE는 가려진 영역은 컨텐츠가 없는
 * 것으로 판단하여 가려진 영역 만큼 줄인다.</li>
 * </ul>
 * 
 * @param {Number}
 *            nWidth 창의 너비
 * @param {Number}
 *            nHeight 창의 높이
 * @example // 새 창을 띄우고 자동으로 창 크기를 컨텐트에 맞게 변경하는 함수 function winopen(url) { try {
 *          win = window.open(url, "",
 *          "toolbar=0,location=0,status=0,menubar=0,scrollbars=0,resizable=0,width=250,height=300");
 *          win.moveTo(200, 100); win.focus(); } catch(e){}
 * 
 * setTimeout(function() { $Window(win).sizeToContent(); }, 1000); }
 * 
